...
Public Module MainApp
    ...
    Public Property VerifyData As Boolean = False 'Holds the verify data flag
    Public Property SPI_MODE As SpiDeviceMode = SpiDeviceMode.SPI
    ...
    Public Property NAND_Manager As Boolean = True 'Indicates if the NAND Block Manager is enabled
    Public Property NAND_Preserve As Boolean = True 'We want to copy SPARE data before erase
    Public Property SO44_VPP As SO44_VPP_SETTING = SO44_VPP_SETTING.Disabled 'Enables the SO-44 Adapter's 12v VPP feature


    Public Const Build As Integer = 425
    ...
    Public WithEvents NAND_IF As New NAND_API
    ...
    Sub Main(ByVal Args() As String)
        'Args = {"-write", "-i2c", "24XX64", "-file", "d:\flash.bin"}
...
#Region "SPI Helper"
    ...
    Public Sub SPI_Detect()
        ...
        If SPI_MODE = SpiDeviceMode.SPI OrElse SPI_MODE = SpiDeviceMode.SQI Then
        ...
        ElseIf SPI_MODE = SpiDeviceMode.AT25512 Then
            Dim AT25512 As New SPI_FLASH("Atmel AT25512 / Microchip 25AA512", 65536, 128) '128 bytes per page/sector
        ...
        ElseIf SPI_MODE = SpiDeviceMode.M95010 Then
            ...
            M95010.ProgramMode = SPI_ProgramMode.SPI_EEPROM_8BIT
            SPI_IF.SPIBUS_Setup()
        ...
        ElseIf SPI_MODE = SpiDeviceMode.M95020 Then
            ...
            M95020.ProgramMode = SPI_ProgramMode.SPI_EEPROM_8BIT
            SPI_IF.SPIBUS_Setup()
        ...
        ElseIf SPI_MODE = SpiDeviceMode.M95040 Then
            ...
            M95040.ProgramMode = SPI_ProgramMode.SPI_EEPROM_8BIT
            SPI_IF.SPIBUS_Setup()
        ...
        ElseIf SPI_MODE = SpiDeviceMode.M95640 Then
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M95128 Then '16384 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M95256 Then '32768 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M95512 Then '65536 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M95M01 Then '131072 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M95M02 Then '262144 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25010A Then '128 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25020A Then '256 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25040A Then '512 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25080 Then '1024 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25160 Then '2048 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25320 Then '4096 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.AT25640 Then '8192 bytes
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M25AA160A Then
            ...
            SPI_IF.MyFlashStatus = ConnectionStatus.Supported
            WriteConsole(RM.GetString("fcusb_spi_usingcustom"))
        ElseIf SPI_MODE = SpiDeviceMode.M25AA160B Then
    ...
    Private Function CreatenRF24LE1() As SPI_FLASH
    ...
    Private Function CreatenRF24LUIP()
        Dim nordic_spi As New SPI_FLASH("Nordic nRF24LUI+", 32768, 256)
        nordic_spi.OP_COMMANDS.SE = &H52
        nordic_spi.ERASE_SIZE = 512
        nordic_spi.ProgramMode = SPI_ProgramMode.Nordic
        Return nordic_spi
...
#Region "Console mode"
    ...
    Private Sub RunConsoleMode(ByVal Args() As String)
        ...
        Dim opt_fileoffset As UInt32 = 0
        ...
        Dim opt_device As SpiDeviceMode = SpiDeviceMode.SPI
                ...
                Select Case Args(i).ToUpper
                    ...
                    Case "-OFFSET"
                        ...
                        Try
                            If IsNumeric(offset_value) Then
                                opt_fileoffset = CUInt(offset_value)
                            ElseIf Utilities.IsDataType.HexString(offset_value) Then
                                opt_fileoffset = Utilities.HexToUInt(offset_value)
                            End If
                        Catch ex As Exception
                        End Try
                    Case "-LENGTH"
                    ...
                    Case "-DEVICE"
                        ...
                        Select Case NextItem.ToUpper
                            Case "AT25128B"
                                opt_device = SpiDeviceMode.AT25128B
                            Case "AT25256B"
                                opt_device = SpiDeviceMode.AT25256B
                            Case "AT25512"
                                opt_device = SpiDeviceMode.AT25512
                            Case "M95010"
                                opt_device = SpiDeviceMode.M95010
                            Case "M95020"
                                opt_device = SpiDeviceMode.M95020
                            Case "M95040"
                                opt_device = SpiDeviceMode.M95040
                            Case "M95080"
                                opt_device = SpiDeviceMode.M95080
                            Case "M95160"
                                opt_device = SpiDeviceMode.M95160
                            Case "M95320"
                                opt_device = SpiDeviceMode.M95320
                            Case "M95640"
                                opt_device = SpiDeviceMode.M95640
                            Case "M95128"
                                opt_device = SpiDeviceMode.M95128
                            Case "M95256"
                                opt_device = SpiDeviceMode.M95256
                            Case "M95512"
                                opt_device = SpiDeviceMode.M95512
                            Case "M95M01"
                                opt_device = SpiDeviceMode.M95M01
                            Case "M95M02"
                                opt_device = SpiDeviceMode.M95M02
                            Case "AT25010A"
                                opt_device = SpiDeviceMode.AT25010A
                            Case "AT25020A"
                                opt_device = SpiDeviceMode.AT25020A
                            Case "AT25040A"
                                opt_device = SpiDeviceMode.AT25040A
                            Case "AT25080"
                                opt_device = SpiDeviceMode.AT25080
                            Case "AT25160"
                                opt_device = SpiDeviceMode.AT25160
                            Case "AT25320"
                                opt_device = SpiDeviceMode.AT25320
                            Case "AT25640"
                                opt_device = SpiDeviceMode.AT25640
                            Case "M25AA160A"
                                opt_device = SpiDeviceMode.M25AA160A
                            Case "M25AA160B"
                                opt_device = SpiDeviceMode.M25AA160B
                            Case Else
        ...
        Select Case OperationMode
            ...
            Case AvrMode.SPI
                ...
                If (opt_device = SpiDeviceMode.SPI) Then
                ...
                Select Case SPI_IF.MyFlashStatus
                    ...
                    Case ConnectionStatus.Supported
                        Dim device_id As String = Hex(SPI_IF.MyFlashDevice.MFG_CODE).PadLeft(2, "0") & " " & Hex(SPI_IF.MyFlashDevice.PART_CODE).PadLeft(4, "0")
                        ConsoleWriteLine(RM.GetString("cnts_device") & ": SPI compatible device (JEDEC ID: " & device_id & ")")
            ...
            Case AvrMode.EXPIO
                ...
                Select Case SPI_IF.MyFlashStatus
                    ...
                    Case ConnectionStatus.NotSupported
                        ...
                        GoTo ExitConsoleMode
                    Case ConnectionStatus.Supported
                        Dim device_id As String = Hex(EXT_IF.MyMPDevice.MFG_CODE).PadLeft(2, "0") & " " & Hex(EXT_IF.MyMPDevice.PART_CODE).PadLeft(4, "0")
                        ConsoleWriteLine(RM.GetString("cnts_device") & ": Multi-purpose Flash device (JEDEC ID: " & device_id & ")")
                        AddMemoryDevice(DeviceTypes.ExtIO, 0, SPI_IF.DeviceSize, Nothing, SPI_IF.DeviceName)
                End Select
            Case AvrMode.NAND
                ConsoleWriteLine("Board is currently in NAND mode.") : GoTo ExitConsoleMode
            Case AvrMode.DFU
        ...
        If (Not task = ConsoleTask.ExecuteScript) AndAlso (Not OperationMode = AvrMode.I2C) Then
            ...
            End If
            If opt_filelength = 0 Then '-LENGTH not specified
                If task = ConsoleTask.ReadMemory Then
                    opt_filelength = MyMemDevices(0).MemSize
                ElseIf task = ConsoleTask.WriteMemory Then
                    opt_filelength = file_io.Length
                End If
            End If
            If (opt_fileoffset > MyMemDevices(0).MemSize) Then opt_fileoffset = 0
            opt_filelength = Math.Min(MyMemDevices(0).MemSize - opt_fileoffset, opt_filelength + opt_fileoffset)
        End If
        If (task = ConsoleTask.ReadMemory) Then
            Dim b() As Byte = Nothing
            If (OperationMode = AvrMode.I2C) Then
                If (opt_fileoffset > opt_i2c_device.Size) Then opt_fileoffset = 0
                If opt_filelength = 0 Then opt_filelength = (opt_i2c_device.Size - opt_fileoffset)
                opt_filelength = Math.Min(opt_i2c_device.Size - opt_fileoffset, opt_filelength + opt_fileoffset)
                ConsoleWriteLine("Reading data from I2C Flash device: " & opt_i2c_device.Name & " (" & Format(opt_i2c_device.Size, "#,###") & " bytes)")
                ConsoleWriteLine("I2C parameters: slave-address 0x" & Hex(opt_i2c_addr) & ", offset: 0x" & Hex(opt_fileoffset) & ", length: " & Format(opt_filelength, "#,###") & " bytes")
                b = SPI_IF.ReadData_I2C_EEPROM(opt_fileoffset, opt_filelength)
            Else
                ...
                MyMemDevices(0).ReadBytes(opt_fileoffset, opt_filelength, FlashArea.Main, cb)
            End If
            ...
            End If
            Utilities.FileIO.WriteBytes(b, file_io.FullName)
        ...
        ElseIf task = ConsoleTask.WriteMemory Then
            If (OperationMode = AvrMode.I2C) Then
                ...
                ConsoleWriteLine("I2C parameters: slave-address 0x" & Hex(opt_i2c_addr) & ", offset: 0x" & Hex(opt_fileoffset) & ", length: " & Format(opt_filelength, "#,###") & " bytes")
                If SPI_IF.WriteData_I2C_EEPROM(opt_fileoffset, data_out) Then
            ...
            Else
                ...
                End If
                If (opt_fileoffset > MyMemDevices(0).MemSize) Then opt_fileoffset = 0
                If opt_filelength = 0 Then opt_filelength = (MyMemDevices(0).MemSize - opt_fileoffset)
                opt_filelength = Math.Min(MyMemDevices(0).MemSize - opt_fileoffset, opt_filelength + opt_fileoffset)
                ReDim Preserve data_out(opt_filelength - 1)
                Dim cb As New MemoryDeviceInstance.StatusCallback
                cb.Progress = New UpdateFunction_Progress(AddressOf SetConsoleProgress)
                cb.Speed = New UpdateFunction_SpeedLabel(AddressOf SetConsoleSpeed)
                FCUSB_FlashWrite(opt_fileoffset, data_out, 0, cb)
            End If
...
#Region "Windows Registry"
    ...
    Public Sub LoadRegistry()
        Try
            ...
            NAND_Manager = GetRegistryValueBool("NandManager", True)
            NAND_Preserve = GetRegistryValueBool("NandPreserve", True)
            SPI_MODE = CInt(GetRegistryValue("SpiMode", "0")) 'Default is normal
            SO44_VPP = CInt(GetRegistryValue("SO44VPP", "1"))
        Catch ex As Exception
            VerifyData = True
        End Try
    End Sub
    ...
    Public Sub SaveRegistry()
        Dim i_mode As Integer = SPI_MODE
        SetRegistryValueBool("VerifyData", VerifyData)
        SetRegistryValueBool("NandManager", NAND_Manager)
        SetRegistryValueBool("NandPreserve", NAND_Preserve)
        SetRegistryValue("SpiMode", CInt(SPI_MODE).ToString)
        SetRegistryValue("SO44VPP", CInt(SO44_VPP).ToString)
    End Sub
...
#End Region
    ...
    Private Function Operation_CheckStatus() As Boolean
        ...
        ElseIf OperationMode = AvrMode.NAND Then
            If Not NAND_IF.IsConnected Then NAND_IF.Disconnect() : GoTo Disconnected
        ElseIf OperationMode = AvrMode.EXPIO Then
            If Not SPI_IF.IsConnected Then SPI_IF.Disconnect() : GoTo Disconnected
        End If
...
Disconnected:
    ...
    Private Function Operation_Connect() As Boolean
        ...
        ElseIf SPI_IF.IsConnected Then
            If GuiForm IsNot Nothing AndAlso (Not AppIsClosing) Then
                GuiForm.ShowSpiSettings(False)
                If SPI_MODE = SpiDeviceMode.I2CEEPROM Then GuiForm.ShowI2CSettings(True)
            End If
            If SPI_IF.Connect Then
                If SPI_MODE = SpiDeviceMode.I2CEEPROM Then
                ...
                If GuiForm IsNot Nothing AndAlso (Not AppIsClosing) Then GuiForm.OnDeviceConnected()
            End If
        ElseIf NAND_IF.IsConnected Then
            If GuiForm IsNot Nothing AndAlso (Not AppIsClosing) Then GuiForm.ShowSpiSettings(False)
            If NAND_IF.Connect() Then
    ...
    Public Enum SpiDeviceMode As Byte
        SPI = 0
        nRF24LE1 = 1
        I2CEEPROM = 2
        AT25128B = 4 '16384 bytes
        AT25256B = 5 '32768 bytes
        AT25512 = 6 '65536 bytes
        M95010 = 7  '128 bytes
        M95020 = 8 '256 bytes
        M95040 = 9 '512 bytes
        M95080 = 10 '1024 bytes
        M95160 = 11 '2048 bytes
        M95320 = 12 '4096 bytes
        M95640 = 13  '8192 bytes
        M95128 = 14 '16384 bytes
        M95256 = 15 '32768 bytes
        M95512 = 16 '65536 bytes
        M95M01 = 17 '131072 bytes
        M95M02 = 18 '262144 bytes
        AT25010A = 19 '128 bytes
        AT25020A = 20 '256 bytes
        AT25040A = 21 '512 bytes
        AT25080 = 22
        AT25160 = 23
        AT25320 = 24
        AT25640 = 25
        M25AA160A = 26 '2048 bytes
        M25AA160B = 27 '2048 bytes
        SQI = 28
        EXT_IO = 29
        nRF24LUIP = 30
        'EXPIO_PLCC32_Normal = 30
        'EXPIO_TSOP_x8 = 32
        'EXPIO_TSOP_x16 = 33
        'EXPIO_TSOP_NAND = 34
    End Enum
