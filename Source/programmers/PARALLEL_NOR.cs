...
namespace FlashcatUSB {
    public class PARALLEL_NOR : FlashcatUSB.MemoryDeviceUSB {
        private FlashcatUSB.USB.FCUSB_DEVICE FCUSB;

        public FlashcatUSB.FlashMemory.P_NOR MyFlashDevice { get; set; }
        public FlashcatUSB.USB.DeviceStatus MyFlashStatus { get; set; } = FlashcatUSB.USB.DeviceStatus.NotDetected;
        public FlashcatUSB.FlashMemory.NOR_CFI CFI { get; set; } // Contains CFI table information (NOR)
        ...
        public PARALLEL_NOR(FlashcatUSB.USB.FCUSB_DEVICE parent_if) {
            FCUSB = parent_if;
        }
        ...
        public bool DeviceInit() {
            ...
            if (DetectFlashDevice()) {
                ...
                FlashcatUSB.FlashMemory.Device[] device_matches;
                if (MyAdapter == FlashcatUSB.MEM_PROTOCOL.NOR_X8) {
                    device_matches = FlashcatUSB.MainApp.FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, 0, FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR);
                } else {
                    device_matches = FlashcatUSB.MainApp.FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, FLASH_IDENT.ID2, FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR);
                }

                CFI = new FlashcatUSB.FlashMemory.NOR_CFI(EXPIO_NOR_GetCFI());
                if (device_matches is object && device_matches.Count() > 0) {
                    ...
                    if (FlashcatUSB.MainApp.MySettings.MULTI_CE > 0) {
                        PrintConsole?.Invoke("Multi-chip select feature is enabled");
                        DUALDIE_EN = true;
                        DUALDIE_CE2 = FlashcatUSB.MainApp.MySettings.MULTI_CE;
                    ...
                    switch (MyFlashDevice.WriteMode) {
                        case FlashcatUSB.FlashMemory.MFP_PRG.Standard: {
                                EXPIO_SETUP_WRITEDATA(E_EXPIO_WRITEDATA.Standard);
                                break;
                            }

                        case FlashcatUSB.FlashMemory.MFP_PRG.IntelSharp: {
                        ...
                        case FlashcatUSB.FlashMemory.MFP_PRG.BypassMode: { // Writes 64 bytes using ByPass sequence
                                EXPIO_SETUP_WRITEDATA(E_EXPIO_WRITEDATA.Bypass);
                                break;
                            }

                        case FlashcatUSB.FlashMemory.MFP_PRG.PageMode: { // Writes an entire page of data (128 bytes etc.)
                                EXPIO_SETUP_WRITEDATA(E_EXPIO_WRITEDATA.Page);
                                break;
                            }

                        case FlashcatUSB.FlashMemory.MFP_PRG.Buffer1: { // Writes to a buffer that is than auto-programmed
                                CURRENT_SECTOR_ERASE = E_EXPIO_SECTOR.Intel;
                                CURRENT_CHIP_ERASE = E_EXPIO_CHIPERASE.Intel;
                        ...
                        case FlashcatUSB.FlashMemory.MFP_PRG.Buffer2: {
        ...
        private void MyFlashDevice_SelectBest(FlashcatUSB.FlashMemory.Device[] device_matches) {
            if (device_matches.Length == 1) {
                MyFlashDevice = (FlashcatUSB.FlashMemory.P_NOR)device_matches[0];
                return;
            }
            ...
            if ((int)device_matches[0].MFG_CODE == 0x1 && (int)device_matches[0].ID1 == 0xAD) { // AM29F016x (we need to figure out which one)
                if (!CFI.IS_VALID) {
                    ...
                    MyFlashDevice = (FlashcatUSB.FlashMemory.P_NOR)device_matches[1];
                    return;
                } // AM29F016D (Uses Bypass programming)
            } else if (CFI.IS_VALID) {
                var flash_dev = new List<FlashcatUSB.FlashMemory.Device>();
                ...
                if (flash_dev.Count == 1) {
                    MyFlashDevice = (FlashcatUSB.FlashMemory.P_NOR)flash_dev[0];
                    return;
                } else {
                    for (int i = 0, loopTo1 = flash_dev.Count - 1; i <= loopTo1; i++) {
                        if ((long)flash_dev[i].PAGE_SIZE == (long)CFI.WRITE_BUFFER_SIZE) {
                            MyFlashDevice = (FlashcatUSB.FlashMemory.P_NOR)flash_dev[i];
                            return;
                        }
                    }
                }
        ...
        private uint GetAddressForMultiDie(ref uint flash_offset, ref uint count, ref uint buffer_size) {
            int die_count = 2; // Multi die only supports 2 (for now)
            ...
            if (die_id != DIE_SELECTED) {
                if (die_id == 0) {
                    FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_ADDRESS_CE, null, 0U);
                } else {
                    FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_ADDRESS_CE, null, (uint)DUALDIE_CE2);
                }
        ...
        public bool SectorErase(uint sector_index, FlashcatUSB.FlashMemory.FlashArea memory_area = FlashcatUSB.FlashMemory.FlashArea.Main) {
            ...
            try {
                if (sector_index == 0L && SectorSize(0U) == MyFlashDevice.FLASH_SIZE) {
                    ...
                    EXPIO_VPP_DISABLE();
                    if (MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.SR1 | MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.SR2 | MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.RYRB) {
                        ...
                        FlashcatUSB.Utilities.Main.Sleep((int)MyFlashDevice.ERASE_DELAY);
        ...
        public bool WriteData(long logical_address, byte[] data_to_write, FlashcatUSB.WriteParameters Params = null) {
            try {
                ...
                for (int i = 0, loopTo = Loops - 1; i <= loopTo; i++) {
                    ...
                    if (!ReturnValue)
                        return false;
                    if (FCUSB.HWBOARD == FlashcatUSB.USB.FCUSB_BOARD.Mach1 && MyFlashDevice.WriteMode == FlashcatUSB.FlashMemory.MFP_PRG.BypassMode) {
                        FlashcatUSB.Utilities.Main.Sleep(300); // Board is too fast! We need a delay between writes (i.e. AM29LV160B)
                    }
                ...
                if (MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.DQ7 | MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.SR1 | MyFlashDevice.DELAY_MODE == FlashcatUSB.FlashMemory.MFP_DELAY.SR2) {
                    EXPIO_WAIT();
                } else {
                    FlashcatUSB.Utilities.Main.Sleep((int)MyFlashDevice.SOFTWARE_DELAY);
                }
            } catch (Exception ex) {
            } finally {
                EXPIO_VPP_DISABLE();
        ...
        public void WaitUntilReady() {
            ...
            // FCUSB.USB_WaitForComplete() 'Checks for WAIT flag to clear
            FlashcatUSB.Utilities.Main.Sleep(100); // Some flash devices have registers, some rely on delays
        }
        ...
        public bool EraseDevice() {
            try {
                try {
                    ...
                    if (wm == FlashcatUSB.FlashMemory.MFP_PRG.IntelSharp | wm == FlashcatUSB.FlashMemory.MFP_PRG.Buffer1) {
                        int BlockCount = (int)MyFlashDevice.Sector_Count;
                        SetProgress?.Invoke(0);
                        for (int i = 0, loopTo = BlockCount - 1; i <= loopTo; i++) {
                            if (!SectorErase((uint)i, (FlashcatUSB.FlashMemory.FlashArea)0)) {
                                SetProgress?.Invoke(0);
                        ...
                        EXPIO_EraseChip();
                        FlashcatUSB.Utilities.Main.Sleep(200); // Perform blank check
                        for (int i = 0; i <= 179; i++) { // 3 minutes
                            if (BlankCheck(0U))
                                return true;
                            FlashcatUSB.Utilities.Main.Sleep(900);
                        }
        ...
        private bool EXPIO_SETUP_USB(FlashcatUSB.MEM_PROTOCOL mode) {
            try {
                ...
                if (result_data[0] == 0x17) { // Extension port returns 0x17 if it can communicate with the MCP23S17
                    ...
                    switch (mode) {
                        case FlashcatUSB.MEM_PROTOCOL.NOR_X8: {
                                CURRENT_BUS_WIDTH = E_BUS_WIDTH.X8;
                                break;
                            }

                        case FlashcatUSB.MEM_PROTOCOL.NOR_X16: {
                                CURRENT_BUS_WIDTH = E_BUS_WIDTH.X16;
                                break;
                            }

                        case FlashcatUSB.MEM_PROTOCOL.NOR_X16_X8: {
                                CURRENT_BUS_WIDTH = E_BUS_WIDTH.X16;
                                break;
                            }
                    }
        ...
        private bool EXPIO_SETUP_WRITEDATA(E_EXPIO_WRITEDATA mode) {
            try {
                bool result = FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_MODE_WRITE, null, (uint)mode);
        ...
        private bool EXPIO_SETUP_DELAY(FlashcatUSB.FlashMemory.MFP_DELAY delay_mode) {
            try {
                bool result = FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_MODE_DELAY, null, (uint)delay_mode);
        ...
        private bool EXPIO_SETUP_WRITEDELAY(ushort delay_cycles) {
            try {
                bool result = FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_DELAY, null, delay_cycles);
                System.Threading.Thread.Sleep(25);
                return result;
            } catch (Exception ex) {
                return false;
        ...
        private void EXPIO_VPP_ENABLE() {
            ...
            if (if_type == FlashcatUSB.FlashMemory.VCC_IF.X16_5V_12VPP) {
                VPP_FEAT_EN = true;
            } else if (if_type == FlashcatUSB.FlashMemory.VCC_IF.X16_3V_12VPP) {
                VPP_FEAT_EN = true;
            } else if (if_type == FlashcatUSB.FlashMemory.VCC_IF.X8_5V_12VPP) {
                VPP_FEAT_EN = true;
            }

            if (VPP_FEAT_EN) {
                this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.VPP_12V);
                FlashcatUSB.Utilities.Main.Sleep(100); // We need to wait
            }
        }
        // We should only allow this for devices that have a 12V option/chip
        ...
        private void EXPIO_PrintCurrentWriteMode() {
            switch (CURRENT_WRITE_MODE) {
                case E_EXPIO_WRITEDATA.Standard: {  // 0x5555=0xAA;0x2AAA=0x55;0x5555=0xA0;SA=DATA;DELAY
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Standard");
                        break;
                    }

                case E_EXPIO_WRITEDATA.Intel: { // SA=0x40;SA=DATA;SR.7
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Auto-Word Program");
                        break;
                    }

                case E_EXPIO_WRITEDATA.Bypass: { // 0x555=0xAA;0x2AA=55;0x555=20;(0x00=0xA0;SA=DATA;...)0x00=0x90;0x00=0x00
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Bypass Mode");
                        break;
                    }

                case E_EXPIO_WRITEDATA.Page: {  // 0x5555,0x2AAA,0x5555;(BA/DATA)
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Page Write");
                        break;
                    }

                case E_EXPIO_WRITEDATA.Buffer_1: {  // 0xE8...0xD0
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Buffer (Intel)");
                        break;
                    }

                case E_EXPIO_WRITEDATA.Buffer_2: { // 0x555=0xAA,0x2AA=0x55,SA=0x25,SA=(WC-1)..
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_write_mode_supported") + ": Buffer (Cypress)");
                        break;
                    }
            }
        }
        ...
        public void EXPIO_SetTiming(int read_access, int we_pulse) {
            if (FCUSB.HWBOARD == FlashcatUSB.USB.FCUSB_BOARD.Mach1) {
                FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_TIMING, null, (uint)(read_access << 8 | we_pulse));
            }
        }
        ...
        private byte[] EXPIO_NOR_GetCFI() {
            try {
                ...
                if (CFI_ExecuteCommand(() => WriteCommandData(0x55U, 0x98))) { // Issue Enter CFI command
                    FlashcatUSB.MainApp.PrintConsole("Common Flash Interface information present");
                    return cfi_data;
                    ...
                    cfi_data = null;
                    FlashcatUSB.MainApp.PrintConsole("Common Flash Interface information not present");
                }
            } catch (Exception ex) {
            } finally {
                EXPIO_ResetDevice();
        ...
        public bool WriteCommandData(uint cmd_addr, ushort cmd_data) {
            ...
            if (FCUSB.HWBOARD == FlashcatUSB.USB.FCUSB_BOARD.Mach1) {
                FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.LOAD_PAYLOAD, addr_data);
                return FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_WRCMDDATA);
            } else {
                return FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_WRCMDDATA, addr_data);
            }
        }

        public ushort WriteMemoryAddress(uint mem_addr, ushort mem_data) {
            var addr_data = new byte[6];
            ...
            if (FCUSB.HWBOARD == FlashcatUSB.USB.FCUSB_BOARD.Mach1) {
                FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.LOAD_PAYLOAD, addr_data);
                return Conversions.ToUShort(FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_WRMEMDATA));
            } else {
                return Conversions.ToUShort(FCUSB.USB_CONTROL_MSG_OUT(FlashcatUSB.USB.USBREQ.EXPIO_WRMEMDATA, addr_data));
            }
        }

        public ushort ReadMemoryAddress(uint mem_addr) {
            var data_out = new byte[2];
            FCUSB.USB_CONTROL_MSG_IN(FlashcatUSB.USB.USBREQ.EXPIO_RDMEMDATA, ref data_out, mem_addr);
            return data_out[1] << 8 | data_out[0];
        }
        ...
        private bool DetectFlashDevice() {
            ...
            LAST_DETECT.MFG = 0;
            FLASH_IDENT = this.DetectFlash(FlashcatUSB.MEM_PROTOCOL.NOR_X16);
            if (FLASH_IDENT.Successful) {
                var d = FlashcatUSB.MainApp.FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, FLASH_IDENT.ID2, FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR);
                if (d.Count() > 0 && this.IsIFACE16X(((FlashcatUSB.FlashMemory.P_NOR)d[0]).IFACE)) {
                    PrintConsole?.Invoke(string.Format(FlashcatUSB.MainApp.RM.GetString("ext_device_detected"), "NOR X16 (Word addressing)"));
                    MyAdapter = FlashcatUSB.MEM_PROTOCOL.NOR_X16;
                    return true;
                } else {
            ...
            FLASH_IDENT = this.DetectFlash(FlashcatUSB.MEM_PROTOCOL.NOR_X16_X8);
            if (FLASH_IDENT.Successful) {
                var d = FlashcatUSB.MainApp.FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, 0, FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR);
                if (d.Count() > 0 && this.IsIFACE16X(((FlashcatUSB.FlashMemory.P_NOR)d[0]).IFACE)) {
                    PrintConsole?.Invoke(string.Format(FlashcatUSB.MainApp.RM.GetString("ext_device_detected"), "NOR X16 (Byte addressing)"));
                    MyAdapter = FlashcatUSB.MEM_PROTOCOL.NOR_X16_X8;
                    return true;
                } else {
                    LAST_DETECT = FLASH_IDENT;
                }
            ...
            FLASH_IDENT = this.DetectFlash(FlashcatUSB.MEM_PROTOCOL.NOR_X8);
            if (FLASH_IDENT.Successful) {
                var d = FlashcatUSB.MainApp.FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, 0, FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR);
                if (d.Count() > 0 && this.IsIFACE8X(((FlashcatUSB.FlashMemory.P_NOR)d[0]).IFACE)) {
                    PrintConsole?.Invoke(string.Format(FlashcatUSB.MainApp.RM.GetString("ext_device_detected"), "NOR X8"));
                    MyAdapter = FlashcatUSB.MEM_PROTOCOL.NOR_X8;
                    return true;
                } else {
                    LAST_DETECT = FLASH_IDENT;
                }
        ...
        private FlashcatUSB.FlashMemory.FlashDetectResult EXPIO_DetectX16() {
            byte[] ident_data = null;
            FlashcatUSB.FlashMemory.Device[] devices = null;
            this.EXPIO_SETUP_USB(FlashcatUSB.MEM_PROTOCOL.NOR_X16);
            ident_data = EXPIO_ReadIdent(true);
            return FlashcatUSB.FlashMemory.Tools.GetFlashResult(ident_data);
        }

        private FlashcatUSB.FlashMemory.FlashDetectResult EXPIO_DetectX16_X8() {
            byte[] ident_data = null;
            FlashcatUSB.FlashMemory.Device[] devices = null;
            this.EXPIO_SETUP_USB(FlashcatUSB.MEM_PROTOCOL.NOR_X16_X8);
            ident_data = EXPIO_ReadIdent(true);
            return FlashcatUSB.FlashMemory.Tools.GetFlashResult(ident_data);
        }

        private FlashcatUSB.FlashMemory.FlashDetectResult EXPIO_DetectX8() {
            byte[] ident_data = null;
            FlashcatUSB.FlashMemory.Device[] devices = null;
            this.EXPIO_SETUP_USB(FlashcatUSB.MEM_PROTOCOL.NOR_X8);
        ...
        private void PrintDeviceInterface() {
            switch (MyFlashDevice.IFACE) {
                case FlashcatUSB.FlashMemory.VCC_IF.X8_3V: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X8 (3V)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X8_5V: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X8 (5V)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X8_5V_12VPP: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X16 (5V/12V VPP)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X16_1V8: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X16 (1.8V)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X16_3V: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X16 (3V)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X16_5V: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X16 (5V)");
                        break;
                    }

                case FlashcatUSB.FlashMemory.VCC_IF.X16_5V_12VPP: {
                        PrintConsole?.Invoke(FlashcatUSB.MainApp.RM.GetString("ext_device_interface") + ": NOR X16 (5V/12V VPP)");
                        break;
                    }
            }
        ...
        private bool IsIFACE8X(FlashcatUSB.FlashMemory.VCC_IF input) {
            switch (input) {
                case FlashcatUSB.FlashMemory.VCC_IF.X8_3V: {
                        return true;
                    }
        ...
        private bool IsIFACE16X(FlashcatUSB.FlashMemory.VCC_IF input) {
            switch (input) {
                case FlashcatUSB.FlashMemory.VCC_IF.X16_1V8: {
                        return true;
                    }
        ...
        public bool ResetDevice() {
            try {
                if (MyFlashDevice.FLASH_TYPE == FlashcatUSB.FlashMemory.MemoryType.PARALLEL_NOR) {
                    EXPIO_ResetDevice();
                }
            } catch (Exception ex) {
                return false;
        ...
        private bool WriteBulk(uint address, byte[] data_out) {
            try {
                ...
                return FCUSB.USB_SETUP_BULKOUT(FlashcatUSB.USB.USBREQ.EXPIO_WRITEDATA, setup_data, data_out, 0U);
            } catch (Exception ex) {
            }
        ...
        public void PARALLEL_PORT_TEST() {
            FlashcatUSB.MainApp.SetStatus("Performing parallel I/O output test");
            this.EXPIO_SETUP_USB(FlashcatUSB.MEM_PROTOCOL.NOR_X16);
            WriteCommandData(0xFFFFFFFFU, 0xFFFF);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.BYTE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.RB0_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.OE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.CE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.WE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.CLE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.ALE_HIGH);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.RELAY_OFF);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.VPP_5V);
            FlashcatUSB.Utilities.Main.Sleep(500);
            WriteCommandData(0U, 0);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.BYTE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.OE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.CE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.WE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.CLE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.ALE_LOW);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.RB0_LOW);
            FlashcatUSB.Utilities.Main.Sleep(500);
            this.HardwareControl(FlashcatUSB.USB.FCUSB_HW_CTRL.VPP_5V);
            FlashcatUSB.Utilities.Main.Sleep(300);
            ...
            WriteCommandData(0U, 0);
            FlashcatUSB.MainApp.SetStatus("Parallel I/O output test complete");
        }
    }
}