...
#End Region
    ...
    Public Class Engine : Implements ECC_Engine
        ...
        Private LastResult As ECC_DECODE_RESULT = ECC_DECODE_RESULT.NoErrors
        ...
        Public Sub ReadData(data_in() As Byte, ecc() As Byte) Implements ECC_Engine.ReadData
            Try
                Dim ecc_result = ECC_DECODE_RESULT.NoErrors
                If Utilities.IsByteArrayFilled(ecc, 255) Then Exit Sub  'ECC area does not contain ECC data
                If Not (data_in.Length Mod 512 = 0) Then
                    LastResult = ECC_DECODE_RESULT.InputError
                    Exit Sub
                End If
                If Me.REVERSE_ARRAY Then Array.Reverse(data_in)
                ...
                For i = 1 To data_in.Length Step 512
                    ...
                    Select Case ecc_mode
                        Case ecc_algorithum.hamming
                            ecc_result = ecc_hamming.ProcessECC(block, ecc_data)
                        Case ecc_algorithum.reedsolomon
                            ecc_result = ecc_reedsolomon.ProcessECC(block, ecc_data)
                        Case ecc_algorithum.bhc
                            ecc_result = ecc_bhc.ProcessECC(block, ecc_data)
                    End Select
                    If ecc_result = ECC_DECODE_RESULT.Uncorractable Then
                        LastResult = ECC_DECODE_RESULT.Uncorractable
                        Exit Sub
                    ElseIf ecc_result = ECC_DECODE_RESULT.Correctable Then
                        Array.Copy(block, 0, data_in, i - 1, 512) 'Copies the correct block into the current page data
                        ecc_result = ECC_DECODE_RESULT.NoErrors
                    End If
                    ecc_ptr += ecc_byte_size
                Next
                LastResult = ECC_DECODE_RESULT.NoErrors
            Catch ex As Exception
            End Try
        End Sub

        Public Sub WriteData(data_in() As Byte, ByRef ecc() As Byte) Implements ECC_Engine.WriteData
            Try
        ...
        Public Function GetEccByteSize() As UInt16 Implements ECC_Engine.GetEccByteSize
            Select Case ecc_mode
                Case ecc_algorithum.hamming
                    Return 3
                Case ecc_algorithum.reedsolomon
        ...
        Public Function GetEccFromSpare(spare() As Byte, page_size As UInt16, oob_size As UInt16) As Byte() Implements ECC_Engine.GetEccFromSpare
        ...
        Public Sub SetEccToSpare(spare() As Byte, ecc_data() As Byte, page_size As UInt16, oob_size As UInt16) Implements ECC_Engine.SetEccToSpare
            Dim bytes_per_ecc As Integer = Me.GetEccByteSize() 'Number of ECC byters per sector
            Dim obb_count As Integer = (spare.Length \ oob_size) 'Number of OOB areas to process
            Dim sector_count As Integer = (page_size \ 512) 'Each OOB contains this many sectors
            Dim ecc_data_ptr As Integer = 0
        ...
        Public Function GetLastResult() As ECC_DECODE_RESULT Implements ECC_Engine.GetLastResult
            Return LastResult
        End Function
