...
#Region "Bytes From"
            ...
            Public Function FromUInt32(int_in As UInt32, Optional compress As Boolean = False) As Byte()
                Dim byteArray As Byte() = BitConverter.GetBytes(int_in)
                Array.Reverse(byteArray) 'Need to reverse the result
                If compress Then
            ...
            Public Function FromInt32(int_in As Int32, Optional compress As Boolean = False) As Byte()
                Dim byteArray As Byte() = BitConverter.GetBytes(int_in)
                Array.Reverse(byteArray) 'Need to reverse the result
                If compress Then
                    Dim ret() As Byte = Nothing
...
#End If
        ...
        Public Function IsNumeric(s As String) As Boolean
            ...
            If s = String.Empty OrElse s Is Nothing Then
                value = False
            Else
                For Each c As Char In s.ToCharArray()
                    value = value AndAlso Char.IsDigit(c)
                Next
            End If
            Return value
        End Function

        Public Function PlatformIDToStr(id As PlatformID) As String
            If id = PlatformID.Win32S Then
                Return "Windows"
            ElseIf id = PlatformID.Win32Windows Then
                Return "Windows 98"
            ElseIf id = PlatformID.Win32NT Then
                Return "Windows"
            ElseIf id = PlatformID.WinCE Then
                Return "Windows CE"
            ElseIf id = PlatformID.Unix Then
                Return "Unix"
            ElseIf id = PlatformID.Xbox Then
                Return "Xbox"
            ElseIf id = PlatformID.MacOSX Then
                Return "MacOSX"
            Else
                Return "Other"
            End If
