...
Public Class DFU_API
    ...
    Private Function FindUsbDevices() As WinUsb.WinUsbRegistry()
        Try
            Dim fcusb_list As UsbRegDeviceList
            Dim devices As New List(Of WinUsb.WinUsbRegistry)
            Dim atmel_dev1 As New UsbDeviceFinder(USB_VID_ATMEL, USB_PID_AT90USB162)
            fcusb_list = UsbDevice.AllDevices.FindAll(atmel_dev1)
    ...
    Friend Function IsConnected() As Boolean
        If FCUSB Is Nothing Then
            Dim fcusb_list() As WinUsb.WinUsbRegistry = FindUsbDevices()
            If fcusb_list Is Nothing Then Return False
            Dim FcusbInUse() As String = GetFlashcatInUse()
            For Each fcusb_item In fcusb_list
                Dim o As String = fcusb_item.DeviceProperties("DeviceID")
                If MyUSBDeviceID = o Then
                    Return True
                ElseIf MyUSBDeviceID = "" Then
                    If FcusbInUse Is Nothing Then 'No other instances, we can use this one
                        Return True
                    Else
                        Dim FoundAvailableFcusb As Boolean = True
                        For Each dev_in_use In FcusbInUse
                            If dev_in_use = o Then FoundAvailableFcusb = False : Exit For
                        Next
    ...
    Private Function OpenUsbDevice() As Boolean
        ...
        If st.Err Or Not st.StatusCode = DFU_STATUS_CODE.OK Then Disconnect() : Return False
        Return True
    End Function
