Public Class HexEditor_v2
    Public Property BaseAddress As Long = 0 'The first address of the hex editor we can see
    ...
    Private ViewLocation As Long 'Contains a chache of the current base address
    Private ViewData() As Byte 'Contains a cache of the data that the editor can see
    Private HexDataBitLength As Integer 'number of bits the left side displays
    Private BaseOffset As Long = 0 'This is to offset the drawing
    Private BaseSize As Long = 0 'Number of bytes this hex view can display
    ...
    'This is what creates the hex viewer
    Public Sub CreateHexViewer(ByVal Offset As Long, ByVal Size As Long)
        BaseOffset = Offset
        BaseSize = Size
        BaseAddress = 0
        Dim MaxAddr As Long = Size + Offset
        If (Size <= &HFFFF) Then
            HexDataBitLength = 16 '0000:
        ElseIf (Size <= &HFFFFFF) Then '000000:
            HexDataBitLength = 24
        ElseIf (Size <= &HFFFFFFFFL) Then '00000000:
            HexDataBitLength = 32
        ElseIf (Size <= &HFFFFFFFFFFL) Then '0000000000: yes 1TB
            HexDataBitLength = 40
        End If
        ScrollBar.Minimum = 1
    ...
    Public Sub UpdateScreen()
        ...
        Else
            ...
            If (BaseSize > MaxDataShown) Then
                ...
                Dim NewValue As Long = Math.Round((ScrollBar.Maximum - ScrollBar.LargeChange) * BarPercent) + 1
                If NewValue < 1 Then NewValue = 1
                If NewValue > ScrollBar.Maximum Then NewValue = ScrollBar.Maximum
                ScrollBar.Value = NewValue
                BaseAddress = (NewValue - 1) * CLng(BytesPerLine)
            Else
            ...
            If BaseAddress < 0 Then BaseAddress = 0
            Dim DataToGet As Long = BaseSize - BaseAddress 'The amount of bytes we need to display in the box
            If (DataToGet > MaxDataShown) Then
                DataToGet = MaxDataShown
            End If
            ReDim ViewData(DataToGet - 1)
            RaiseEvent RequestData(BaseAddress, ViewData)
            If ViewData IsNot Nothing Then
                ...
                For i = 0 To LinesToDraw - 1
                    ...
                    End If
                    Array.Copy(ViewData, AddrIndex, BytesForLine, 0, BytesForLine.Length)
                    Drawline(i, BaseAddress + AddrIndex + BaseOffset, BytesPerLine, BytesForLine, gfx)
            ...
            newBG = Nothing
            InRefresh = False
            If (Not BaseAddress = CurrentTop) Then
                CurrentTop = BaseAddress
