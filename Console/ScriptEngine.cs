...
using USB;
using static FlashcatScript.DataTypeObject;
...
namespace FlashcatScript {
    ...
    public class Processor : IDisposable {
        ...
        public event WriteConsoleEventHandler PrintConsole;
        ...
        internal bool ExecuteCommand(string cmd_line) {
            ...
            if (scripe_line.HAS_ERROR) {
                PrintConsole?.Invoke("Error: " + scripe_line.ERROR_MSG);
                return false;
            } else {
                ExitMode argexit_task = default;
                if (!ExecuteScriptElement(scripe_line, ref argexit_task)) {
                    PrintConsole?.Invoke("Error: " + scripe_line.ERROR_MSG);
                }
            }
            return true;
        ...
        internal bool Unload() {
            ...
            for (int i = 0, loopTo = OurFlashDevices.Count - 1; i <= loopTo; i++) {
                ...
                //if (MainApp.GUI is object)
                    //MainApp.GUI.RemoveTab(this_devie);
                MainApp.MEM_IF.Remove(this_devie);
            }
            //if (MainApp.GUI is object)
                //MainApp.GUI.RemoveUserTabs();
                ...
                for (int i = 0, loopTo = e.Length - 1; i <= loopTo; i++) {
                    switch (e[i].ElementType) {
                        case ScriptFileElementType.ELEMENT: {
                                ExecuteScriptElement((ScriptElement)e[i], ref @params.exit_task);
                                @params.err_reason = ((ScriptElement)e[i]).ERROR_MSG;
                                if (!string.IsNullOrEmpty(@params.err_reason)) {
                        ...
                        case ScriptFileElementType.IF_CONDITION: {
                                ...
                                if (test_condition is null || se.CONDITION.HAS_ERROR) {
                                    @params.err_reason = se.CONDITION.ERROR_MSG;
                                    @params.err_line = se.INDEX;
                                    return false;
                                }

                                bool result = (bool)(test_condition.Value);
                                if (se.NOT_MODIFIER)
                                    result = !result;
                                bool execute_result;
                                if (result) {
        ...
        internal bool ExecuteScriptElement(ScriptElement e, ref ExitMode exit_task) {
            try {
                var sv = e.Compile(ref exit_task);
                if (e.HAS_ERROR)
                    return false;
                if (sv is null)
                    return true; // Compiled successfully but no value to save
                if (!string.IsNullOrEmpty(e.TARGET_NAME) && !(e.TARGET_OPERATION == TargetOper.NONE)) {
                    if (!string.IsNullOrEmpty(e.TARGET_VAR)) {
                        if (CurrentVars.IsVariable(e.TARGET_VAR) && CurrentVars.GetVariable(e.TARGET_VAR).Data.VarType == DataType.UInteger) {
                            e.TARGET_INDEX = (int)(CurrentVars.GetVariable(e.TARGET_VAR).Value); // Gets the variable and assigns it to the index
                        } else {
                            e.ERROR_MSG = "Target index is not an integer or integer variable";
                            return false;
                        }
                    }
                    if (e.TARGET_INDEX > -1) { // We are assinging this result to an index within a data array
                        ...
                        if (current_var is null) {
                            e.ERROR_MSG = "Target index used on a variable that does not exist";
                            return false;
                        }
                        if (current_var.Data.VarType == DataType.NULL) {
                            e.ERROR_MSG = "Target index used on a variable that does not yet exist";
                            return false;
                        }
                        if (!(current_var.Data.VarType == DataType.Data)) {
                            e.ERROR_MSG = "Target index used on a variable that is not a DATA array";
                            return false;
                        }
                        byte[] data_out = (byte[])current_var.Value;
                        if (sv.Data.VarType == DataType.UInteger) {
                            byte byte_out = (byte)((uint)sv.Value & 0xFF);
                        ...
                        if (existing_var is null || existing_var.Data.VarType == DataType.NULL) {
                            ...
                            string argerror_reason = e.ERROR_MSG;
                            var result_var = Tools.CompileSVars(existing_var, new_var, var_op, ref argerror_reason);
                            e.ERROR_MSG = argerror_reason;
                            if (!e.ERROR_MSG.Equals(""))
                                return false;
        ...
        private void ProgressUpdate_Status(string msg) {
        ...
        private void CallEvent(string EventName) {
            ...
            var se = GetScriptEvent((string)(EventName));
            if (se is object) {
                ExitMode argexit_task = default;
        ...
        internal ScriptVariable c_str_upper(ScriptVariable[] arguments, uint Index) {
            string input = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_str_hex(ScriptVariable[] arguments, uint Index) {
            int input = (int)(arguments[0].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.String);
        ...
        internal ScriptVariable c_data_new(ScriptVariable[] arguments, uint Index) {
            int size = (Int32)arguments[0].Value; // Size of the Data Array
        ...
        internal ScriptVariable c_data_resize(ScriptVariable[] arguments, uint Index) {
            ...
            int copy_index = (int)arguments[1].Value;
            int copy_length = data_arr.Length - copy_index;
            if (arguments.Length == 3)
                copy_length = (int)arguments[2].Value;
        ...
        internal ScriptVariable c_data_hword(ScriptVariable[] arguments, uint Index) {
            byte[] data1 = (byte[])arguments[0].Value;
            int offset = (int)arguments[1].Value;
            var b = new byte[2];
            b[0] = data1[offset];
            b[1] = data1[offset + 1];
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Integer);
        ...
        internal ScriptVariable c_data_copy(ScriptVariable[] arguments, uint Index) {
            byte[] data1 = (byte[])arguments[0].Value;
            uint src_ind = (uint)(arguments[1].Value);
            uint data_len = (uint)(data1.Length - src_ind);
            if (arguments.Length > 2) {
                data_len = (uint)(arguments[3].Value);
            }
        ...
        internal ScriptVariable c_io_open(ScriptVariable[] arguments, uint Index) {
            ...
            string opt_path = @"\";
            if (arguments.Length > 0)
                title = (string)arguments[0].Value;
            if (arguments.Length > 1)
                filter = (string)arguments[1].Value;
            if (arguments.Length > 2)
                opt_path = (string)arguments[2].Value;
        ...
        internal ScriptVariable c_io_save(ScriptVariable[] arguments, uint Index) {
            ...
            string default_file = "";
            if (arguments.Length > 1) {
                prompt_text = (string)arguments[1].Value;
            }
            if (arguments.Length > 2) {
                default_file = (string)arguments[2].Value;
            }
        ...
        internal ScriptVariable c_io_write(ScriptVariable[] arguments, uint Index) {
            byte[] data1 = (byte[])arguments[0].Value;
            string destination = (string)(arguments[1].Value);
        ...
        internal ScriptVariable c_mem_write(ScriptVariable[] arguments, uint Index) {
            ...
            if (arguments.Length > 2)
                data_len = (int)(arguments[2].Value);
        ...
        internal ScriptVariable c_mem_read(ScriptVariable[] arguments, uint Index) {
            ...
            int offset = (int)(arguments[0].Value);
            int count = (int)(arguments[1].Value);
            bool display = true;
            if (arguments.Length > 2)
                display = (bool)(arguments[2].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Data);
            var cb = new MemoryInterface.MemoryDeviceInstance.StatusCallback();
            if (display) {
                cb.UpdatePercent = new UpdateFunction_Progress(MainApp.ProgressBar_Percent);
        ...
        internal ScriptVariable c_mem_readverify(ScriptVariable[] arguments, uint Index) {
            ...
            int FlashAddress = (int)(arguments[0].Value);
            int FlashLen = (int)(arguments[1].Value);
            byte[] data = null;
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Data);
            MainApp.ProgressBar_Percent(0);
            mem_device.DisableGuiControls();
        ...
        internal ScriptVariable c_mem_sectorsize(ScriptVariable[] arguments, uint Index) {
            ...
            int sector_int = (int)(arguments[0].Value);
        ...
        internal ScriptVariable c_mem_erasesector(ScriptVariable[] arguments, uint Index) {
            ...
            int mem_sector = (int)(arguments[0].Value);
            mem_device.EraseSector((uint)mem_sector);
            long target_addr = mem_device.GetSectorBaseAddress((uint)mem_sector);
            string target_area = "0x" + target_addr.ToString("X").PadLeft(8, '0') + " to 0x" + 
                (target_addr + (long)mem_device.GetSectorSize((uint)mem_sector) - 1L).ToString("X").PadLeft(8, '0');
        ...
        internal ScriptVariable c_spi_clock(ScriptVariable[] arguments, uint Index) {
            int clock_int = (int)(arguments[0].Value);
            MainApp.MySettings.SPI_CLOCK_MAX = (SPI_SPEED)clock_int;
            if ((long)MainApp.MySettings.SPI_CLOCK_MAX < 1000000L)
                MainApp.MySettings.SPI_CLOCK_MAX = (SPI_SPEED)1000000;
            return null;
        }

        internal ScriptVariable c_spi_mode(ScriptVariable[] arguments, uint Index) {
            int mode_int = (int)(arguments[0].Value);
            switch (mode_int) {
                case 0: {
                        MainApp.MySettings.SPI_MODE = SPI_CLOCK_POLARITY.SPI_MODE_0;
                        break;
                    }

                case 1: {
                        MainApp.MySettings.SPI_MODE = SPI_CLOCK_POLARITY.SPI_MODE_1;
                        break;
                    }

                case 2: {
                        MainApp.MySettings.SPI_MODE = SPI_CLOCK_POLARITY.SPI_MODE_2;
                        break;
                    }

                case 3: {
                        MainApp.MySettings.SPI_MODE = SPI_CLOCK_POLARITY.SPI_MODE_3;
                        break;
                    }
            }
        ...
        internal ScriptVariable c_spi_database(ScriptVariable[] arguments, uint Index) {
            ...
            if (arguments.Length == 1) {
                DisplayJedecID = (bool)(arguments[0].Value);
            }
        ...
        internal ScriptVariable c_spi_getsr(ScriptVariable[] arguments, uint Index) {
            ...
            int bytes_to_read = 1;
            if (arguments.Length > 0)
                bytes_to_read = (int)(arguments[0].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Data);
            if (CURRENT_DEVICE_MODE == DeviceMode.SPI) {
        ...
        internal ScriptVariable c_spi_writeread(ScriptVariable[] arguments, uint Index) {
            ...
            int ReadBack = 0;
            if (arguments.Length == 2)
                ReadBack = (int)(arguments[1].Value);
            if (ReadBack == 0) {
        ...
        internal ScriptVariable c_spi_prog(ScriptVariable[] arguments, uint Index) {
            ...
            int state = (int)(arguments[0].Value);
        ...
        internal ScriptVariable c_jtag_config(ScriptVariable[] arguments, uint Index) {
            if (arguments.Length == 1) {
                switch (((string)(arguments[0].Value)).ToUpper()) {
                    ...
                    default: {
                            return new ScriptVariable("ERROR", DataType.FncError) { Value = "Unknown mode: " + (string)arguments[0].Value };
                        }
                }
            } else {
        ...
        internal ScriptVariable c_jtag_select(ScriptVariable[] arguments, uint Index) {
            MainApp.Select_JTAG_Device((int)(arguments[0].Value));
            return null;
        }
        ...
        internal ScriptVariable c_jtag_set(ScriptVariable[] arguments, uint Index) {
            int jtag_device_index = (int)(arguments[0].Value);
            string bsdl_name = (string)(arguments[1].Value);
        ...
        internal ScriptVariable c_jtag_add(ScriptVariable[] arguments, uint Index) {
            string bsdl_lib = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_jtag_control(ScriptVariable[] arguments, uint Index) {
            uint control_value = (uint)(arguments[0].Value);
        ...
        internal ScriptVariable c_jtag_memoryinit(ScriptVariable[] arguments, uint Index) {
            string flash_type = (string)arguments[0].Value;
            MemoryInterface.MemoryDeviceInstance new_dev = default;
            switch (flash_type.ToUpper() ?? "") {
                case "CFI": {
                        uint base_address = (uint)arguments[1].Value;
                        ...
                        if (MainApp.MAIN_FCUSB.JTAG_IF.CFI_Detect(base_address)) {
                            new_dev = MainApp.Connected_Event(MainApp.MAIN_FCUSB, 16384);
                        } else {
                ...
                case "SPI": {
                        ...
                        if (MainApp.MAIN_FCUSB.JTAG_IF.SPI_Detect((JTAG.JTAG_IF.JTAG_SPI_Type)arguments[1].Value)) {
                            new_dev = MainApp.Connected_Event(MainApp.MAIN_FCUSB, 16384);
                        } else {
        ...
        internal ScriptVariable c_jtag_debug(ScriptVariable[] arguments, uint Index) {
            bool enable = (bool)(arguments[0].Value);
            if (enable) {
        ...
        internal ScriptVariable c_jtag_runsvf(ScriptVariable[] arguments, uint Index) {
            if (MainApp.MAIN_FCUSB is null || !MainApp.MAIN_FCUSB.JTAG_IF.Chain_IsValid) {
                return new ScriptVariable("ERROR", DataType.FncError) { Value = "JTAG operations are not valid" };
            }
            MainApp.ProgressBar_Percent(0);
            global::MainApp.MAIN_FCUSB.JTAG_IF.JSP.Progress -= MainApp.ProgressBar_Percent;
            global::MainApp.MAIN_FCUSB.JTAG_IF.JSP.Progress += MainApp.ProgressBar_Percent;
        ...
        internal ScriptVariable c_jtag_shiftdr(ScriptVariable[] arguments, uint Index) {
            ...
            int bit_count = (int)(arguments[1].Value);
            byte[] data_out = null;
            if (arguments.Length == 3)
                exit_mode = (bool)(arguments[2].Value);
        ...
        internal ScriptVariable c_jtag_shiftir(ScriptVariable[] arguments, uint Index) {
            ...
            byte[] data_in = (byte[])arguments[0].Value;
            if (arguments.Length == 2)
                exit_mode = (bool)(arguments[1].Value);
        ...
        internal ScriptVariable c_jtag_shiftout(ScriptVariable[] arguments, uint Index) {
            ...
            bool exit_tms = true;
            if (arguments.Length == 3)
                exit_tms = (bool)(arguments[2].Value);
            byte[] tdo_data = null;
        ...
        internal ScriptVariable c_jtag_write32(ScriptVariable[] arguments, uint Index) {
            ...
            uint addr32 = (uint)(arguments[0].Value);
            uint data = (uint)(arguments[1].Value);
        ...
        internal ScriptVariable c_jtag_state(ScriptVariable[] arguments, uint Index) {
            ...
            string state_str = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_jtag_graycode(ScriptVariable[] arguments, uint Index) {
            ...
            int table_ind = (int)(arguments[0].Value);
            if (arguments.Length == 2)
                use_reserve = (bool)(arguments[1].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.UInteger);
            if (use_reserve) {
        ...
        internal ScriptVariable c_jtag_setdelay(ScriptVariable[] arguments, uint Index) {
            int dev_ind = (int)(arguments[0].Value);
            int delay_val = (int)(arguments[1].Value);
            switch (dev_ind) {
                case 1: { // Intel
                        MainApp.MAIN_FCUSB.USB_CONTROL_MSG_OUT(USBREQ.JTAG_SET_OPTION, null, (uint)((delay_val << 16) + 1));
                        break;
                    }
                case 2: { // AMD
                        MainApp.MAIN_FCUSB.USB_CONTROL_MSG_OUT(USBREQ.JTAG_SET_OPTION, null, (uint)((delay_val << 16) + 2));
                        break;
                    }
                case 3: { // DMA
                        MainApp.MAIN_FCUSB.USB_CONTROL_MSG_OUT(USBREQ.JTAG_SET_OPTION, null, (uint)((delay_val << 16) + 3));
                        break;
                    }
            }
            return null;
        ...
        internal ScriptVariable c_jtag_exitstate(ScriptVariable[] arguments, uint Index) {
            bool exit_state = (bool)(arguments[0].Value);
        ...
        internal ScriptVariable c_bsdl_new(ScriptVariable[] arguments, uint Index) {
            ...
            n.PART_NAME = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_bsdl_find(ScriptVariable[] arguments, uint Index) {
            string param_name = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_bsdl_param(ScriptVariable[] arguments, uint Index) {
            string param_name = (string)(arguments[0].Value);
            uint param_value = (uint)(arguments[1].Value);
        ...
        internal ScriptVariable c_bsp_setup(ScriptVariable[] arguments, uint Index) {
            ...
            if (!(MainApp.MAIN_FCUSB.HWBOARD == FCUSB_BOARD.Professional_PCB5)) {
        ...
        internal ScriptVariable c_bsp_init(ScriptVariable[] arguments, uint Index) {
            ...
            if (arguments.Length == 1)
                flash_mode = (int)(arguments[0].Value);
            bool result = MainApp.MAIN_FCUSB.JTAG_IF.BoundaryScan_Init((bool)(flash_mode==1));
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Bool);
            sv.Value = result;
            return sv;
        }
        ...
        internal ScriptVariable c_bsp_addpin(ScriptVariable[] arguments, uint Index) {
            string pin_name = (string)(arguments[0].Value);
            int pin_output = (int)(arguments[1].Value); // cell associated with the bidir or output cell
            int pin_control = (int)(arguments[2].Value);  // cell associated with the control register bit
            int pin_input = -1; // cell associated with the input cell when output cell is not bidir
            if (arguments.Length == 4) {
                pin_input = (int)(arguments[3].Value);
            }
        ...
        internal ScriptVariable c_bsp_setbsr(ScriptVariable[] arguments, uint Index) {
            int pin_output = (int)(arguments[0].Value);
            int pin_control = (int)(arguments[1].Value);
            bool pin_level = (bool)(arguments[2].Value);
        ...
        internal ScriptVariable c_load_firmware(ScriptVariable[] arguments, uint Index) {
            ...
            MainApp.MAIN_FCUSB.USB_CONTROL_MSG_OUT(USBREQ.FW_REBOOT, null, 0xFFFFFFFFU);
            return null;
        }
        ...
        internal ScriptVariable c_flash_add(ScriptVariable[] arguments, uint Index) {
            string flash_name = (string)(arguments[0].Value);
            int ID_MFG = (int)(arguments[1].Value);
            int ID_PART = (int)(arguments[2].Value);
            int flash_size = (int)(arguments[3].Value); // Upgrade this to LONG in the future
            int flash_if = (int)(arguments[4].Value);
            int block_layout = (int)(arguments[5].Value);
            int prog_mode = (int)(arguments[6].Value);
            int delay_mode = (int)(arguments[7].Value);
        ...
        internal ScriptVariable c_parallel_command(ScriptVariable[] arguments, uint Index) {
            uint cmd_addr = (uint)(arguments[0].Value);
            ushort cmd_data = (ushort)((uint)(arguments[1].Value) & 0xFFFFL);
        ...
        internal ScriptVariable c_msgbox(ScriptVariable[] arguments, uint Index) {
            ...
            if (arguments[0].Data.VarType == DataType.Data) {
                ...
                message_text = (string)(arguments[0].Value);
            }
        ...
        internal ScriptVariable c_writeline(ScriptVariable[] arguments, uint Index) {
            if (arguments[0].Data.VarType == DataType.Data) {
                ...
                if (arguments.Length > 1) {
                    display_addr = (bool)(arguments[1].Value);
                }
                ...
                string message_text = (string)(arguments[0].Value);
        ...
        internal ScriptVariable c_sleep(ScriptVariable[] arguments, uint Index) {
            int wait_ms = (int)(arguments[0].Value);
        ...
        internal ScriptVariable c_verify(ScriptVariable[] arguments, uint Index) {
            bool verify_bool = (bool)(arguments[0].Value);
        ...
        internal ScriptVariable c_ask(ScriptVariable[] arguments, uint Index) {
            string the_question = (string)(arguments[0].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Bool);
        ...
        internal ScriptVariable c_catalog(ScriptVariable[] arguments, uint Index) {
            return null;    //Not supported on linux
        }

        internal ScriptVariable c_cpen(ScriptVariable[] arguments, uint Index) {
            bool cp_en = (bool)(arguments[0].Value);
            if (!MainApp.MAIN_FCUSB.IS_CONNECTED) {
                return new ScriptVariable("ERROR", DataType.FncError) { Value = "FlashcatUSB device is not connected" };
            }
            int w_index = 0;
            if (cp_en)
                w_index = 1;
            MainApp.MAIN_FCUSB.USB_CONTROL_MSG_OUT(USBREQ.EXPIO_CPEN, null, (uint)w_index);
            if (cp_en) {
        ...
        internal ScriptVariable c_cint(ScriptVariable[] arguments, uint Index) {
            uint value = (uint)(arguments[0].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.Integer);
            sv.Value = (int)value;
            return sv;
        }
        ...
        internal ScriptVariable c_cuint(ScriptVariable[] arguments, uint Index) {
            int value = (int)(arguments[0].Value);
            var sv = new ScriptVariable(CurrentVars.GetNewName(), DataType.UInteger);
            sv.Value = (uint)value;
            return sv;
        }
    ...
    internal class ScriptElementOperand {
        ...
        public ScriptElementOperand(Processor oParent, string oper_text) {
            MyParent = oParent;
            Parse(oper_text.Trim());
        }

        private void Parse(string text_input) {
            while (!text_input.Equals("")) {
                if (text_input.StartsWith("(")) {
                    ...
                    if (string.IsNullOrEmpty(x.SubOperands.ERROR_MSG)) {
                        OPERANDS.Add(x);
                    } else {
                        ERROR_MSG = x.SubOperands.ERROR_MSG;
                        return;
                    }
                } else {
                    ...
                    if (MyParent.CmdFunctions.IsScriptFunction(main_element)) {
                        ...
                        OPERANDS.Add(new ScriptElementOperandEntry(MyParent, DataType.Bool, Boolean.Parse(main_element)));
                    } else if (Utilities.IsDataType.Integer(main_element)) {
                        OPERANDS.Add(new ScriptElementOperandEntry(MyParent, DataType.Integer, Int32.Parse(main_element)));
                    } else if (Utilities.IsDataType.Uinteger(main_element)) {
                        OPERANDS.Add(new ScriptElementOperandEntry(MyParent, DataType.UInteger, UInt32.Parse(main_element)));
                    } else if (main_element.EndsWith("U") && Utilities.IsDataType.Uinteger(main_element.Substring(0, main_element.Length - 1))) {
                        OPERANDS.Add(new ScriptElementOperandEntry(MyParent, DataType.UInteger, UInt32.Parse(main_element.Substring(0, main_element.Length - 1))));
                        ...
                        ERROR_MSG = "Unknown function or command: " + text_input;
                        return;
                    } else {
                        ERROR_MSG = "Unknown function or command: " + main_element;
                        return;
                    }
        ...
        private ScriptElementOperand[] ParseArguments(string argument_line) {
            ...
            for (int i = 0, loopTo = argument_line.Length - 1; i <= loopTo; i++) {
                char c = char.Parse(argument_line.Substring(i, 1));
                ...
                if (add_clear) {
                    var n = new ScriptElementOperand(MyParent, arg_builder.ToString());
                    if (!n.ERROR_MSG.Equals("")) {
        ...
        internal ScriptVariable CompileToVariable(ref ExitMode exit_task) {
            ...
            int x = 0;
            while (x < arg_count) {
                ScriptVariable new_var = null;
                if (OPERANDS[x].EntryType == ScriptElementDataType.Operator) {
                    ERROR_MSG = "Expected value to compute";
                    return null;
                }
                new_var = OPERANDS[x].Compile(ref exit_task);
                if (!string.IsNullOrEmpty(OPERANDS[x].ERROR_MSG)) {
                    ERROR_MSG = OPERANDS[x].ERROR_MSG;
                    return null;
                }
                if (!(current_oper == OperandOper.NOTSPECIFIED)) {
                    string argerror_reason = ERROR_MSG;
                    var result_var = Tools.CompileSVars(current_var, new_var, current_oper, ref argerror_reason);
                    ERROR_MSG = argerror_reason;
                    if (!string.IsNullOrEmpty(ERROR_MSG))
                        return null;
                ...
                if (x < arg_count) { // There are more items
                    if (!(OPERANDS[x].Oper == OperandOper.NOTSPECIFIED)) {
                        ...
                        x += 1;
                        if (!(x < arg_count)) {
                            ERROR_MSG = "Statement ended in an operand operation";
                            return null;
                        }
                    } else {
                        ERROR_MSG = "Expected an operand operation";
                        return null;
                    }
                }
            }
    ...
    internal class ScriptElementOperandEntry {
                ...
                case ScriptElementDataType.Data: {
                        switch (FUNC_DATA.VarType) {
                            case DataType.Integer: {
                                    return ((int)FUNC_DATA.Value).ToString();
                                }
                            case DataType.UInteger: {
                                    return ((uint)FUNC_DATA.Value).ToString();
                                }
                            case DataType.String: {
                                    return "\"" + (string)(FUNC_DATA.Value) + "\"";
                                }
                            case DataType.Data: {
                                    return Utilities.Bytes.ToPaddedHexString((byte[])FUNC_DATA.Value);
                                }
                            case DataType.Bool: {
                                    switch ((bool)(FUNC_DATA.Value)) {
                                        case true: {
                                                return "True";
                                            }
                                        case false: {
                            ...
                            case DataType.FncError: {
                                    return "Error: " + (string)(FUNC_DATA.Value);
                                }
                        }
                        break;
                    }
        ...
        internal ScriptVariable Compile(ref ExitMode exit_task) {
            switch (EntryType) {
                        ...
                        if (MyParent.CmdFunctions.GetScriptFunction(FUNC_NAME, FUNC_SUB, ref fnc_params, ref fnc)) {
                            ...
                            if (FUNC_ARGS is object) {
                                for (int i = 0, loopTo = FUNC_ARGS.Length - 1; i <= loopTo; i++) {
                                    ...
                                    if (!string.IsNullOrEmpty(FUNC_ARGS[i].ERROR_MSG)) {
                                        ERROR_MSG = FUNC_ARGS[i].ERROR_MSG;
                                        return null;
                                    }
                            ...
                            if (!CheckFunctionArguments(fnc_params, ref args_var))
                                return null;
                            try {
                                ERROR_MSG = "";
                                uint func_index = 0U;
                                if (Information.IsNumeric(FUNC_IND)) {
                                    func_index = UInt32.Parse(FUNC_IND);
                                    ...
                                    if (v.Data.VarType == DataType.Integer) {
                                        func_index = (uint)MyParent.CurrentVars.GetVariable(FUNC_IND).Value;
                                    } else if (v.Data.VarType == DataType.UInteger) {
                                        func_index = (uint)(MyParent.CurrentVars.GetVariable(FUNC_IND).Value);
                                    } else {
                                        ERROR_MSG = "Index " + FUNC_IND + " must be either an Integer or UInteger";
                                        return null;
                                    }
                                } else {
                                    ERROR_MSG = "Unable to evaluate index: " + FUNC_IND;
                                    return null;
                                }

                                ScriptVariable result = null;
                                try {
                                    result = (ScriptVariable)fnc.DynamicInvoke(args_var, func_index);
                                } catch {
                                ...
                                if (result is null)
                                    return null;
                                if (result.Data.VarType == DataType.FncError) {
                                    ERROR_MSG = (string)(result.Value);
                                    return null;
                                }
                            ...
                            ERROR_MSG = "Unknown function or sub procedure";
                        }
                ...
                case ScriptElementDataType.Event: {
                        ...
                        if (se is null) {
                            ERROR_MSG = "Event does not exist: " + FUNC_NAME;
                            return null;
                        }
                            ...
                            try {
                                if (FUNC_ARGS.Length == 1) {
                                    ...
                                    if (data_index_var.Data.VarType == DataType.UInteger) {
                                        ...
                                        uint data_index = (uint)(data_index_var.Value);
                                ...
                                ERROR_MSG = "Error processing variable index value";
                            }
                        } else {
                            return n_sv;
                        }
                ...
                case ScriptElementDataType.SubItems: {
                        var output_vars = SubOperands.CompileToVariable(ref exit_task);
                        if (!string.IsNullOrEmpty(SubOperands.ERROR_MSG)) {
                            ERROR_MSG = SubOperands.ERROR_MSG;
                            return null;
                        }
                        return output_vars;
                    }
        ...
        private bool CheckFunctionArguments(CmdPrm[] fnc_params, ref ScriptVariable[] my_vars) {
            uint var_count = 0U;
            ...
            if (fnc_params is null && !(var_count == 0L)) {
                ERROR_MSG = "Function " + GetFuncString() + ": arguments supplied but none are allowed";
                return false;
                ...
                for (int i = 0, loopTo = fnc_params.Length - 1; i <= loopTo; i++) {
                    switch (fnc_params[i]) {
                        case CmdPrm.Integer: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires an Integer type parameter";
                                    return default;
                                } else {
                                    if (my_vars[i].Data.VarType == DataType.UInteger) { // We can possibly auto convert data type
                                        uint auto_con = (uint)(my_vars[i].Value);
                                    ...
                                    if (!(my_vars[i].Data.VarType == DataType.Integer)) {
                                        ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs an Integer but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                        return default;
                                    }
                                }
                                break;
                        ...
                        case CmdPrm.UInteger: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires an UInteger type parameter";
                                    return default;
                                } else {
                                    if (my_vars[i].Data.VarType == DataType.Integer) { // We can possibly auto convert data type
                                        int auto_con = (int)(my_vars[i].Value);
                                        if (auto_con >= 0)
                                            my_vars[i] = new ScriptVariable(my_vars[i].Name, DataType.UInteger, (uint)auto_con);
                                    }
                                    if (!(my_vars[i].Data.VarType == DataType.UInteger)) {
                                        ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs an UInteger but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                        return default;
                                    }
                                }
                                break;
                        ...
                        case CmdPrm.String: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires a String type parameter";
                                    return default;
                                } else if (!(my_vars[i].Data.VarType == DataType.String)) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs a String but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                    return default;
                                }
                                break;
                            }
                        case CmdPrm.Data: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires a Data type parameter";
                                    return default;
                                    ...
                                    c.Value = Utilities.Bytes.FromUInt32((uint)(my_vars[i].Value));
                                    my_vars[i] = c;
                                } else {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs an Data but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                    return default;
                                }
                                break;
                            }
                        case CmdPrm.Bool: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires a Bool type parameter";
                                    return default;
                                } else if (!(my_vars[i].Data.VarType == DataType.Bool)) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs an Bool but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                    return default;
                                }
                                break;
                            }
                        case CmdPrm.Any: {
                                if (i >= var_count) {
                                    ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " requires a parameter";
                                    return default;
                                }
                        ...
                        case CmdPrm.String_Optional: {
                                if (!(i >= var_count)) { // Only check if we have supplied this argument
                                    if (!(my_vars[i].Data.VarType == DataType.String)) {
                                        ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs an String but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                        return default;
                                    }
                                }
                                break;
                        ...
                        case CmdPrm.Data_Optional: {
                                if (!(i >= var_count)) { // Only check if we have supplied this argument
                                    if (!(my_vars[i].Data.VarType == DataType.Data)) {
                                        ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs Data but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                        return default;
                                    }
                                }
                                break;
                        ...
                        case CmdPrm.Bool_Optional: {
                                if (!(i >= var_count)) { // Only check if we have supplied this argument
                                    if (!(my_vars[i].Data.VarType == DataType.Bool)) {
                                        ERROR_MSG = "Function " + GetFuncString() + ": argument " + (i + 1) + " inputs Bool but was supplied a " + Tools.GetDataTypeString(my_vars[i].Data.VarType);
                                        return default;
                                    }
                                }
                                break;
    ...
    internal class ScriptFile {
                ...
                while (line_pointer < lines.Length) {
                    ...
                    if (!string.IsNullOrEmpty(cmd_line)) {
                        if (cmd_line.ToUpper().StartsWith("IF ")) { // We are doing an if condition
                            ...
                            var normal = new ScriptElement(MyParent);
                            ...
                            if (!string.IsNullOrEmpty(normal.TARGET_NAME)) { // This element creates a new variable
        ...
        internal ScriptLineElement CreateIfCondition(ref int Pointer, string[] lines, uint[] ind, ref uint ErrInd, ref string ErrorMsg) {
            ...
            if (!this_if.Parse(lines[Pointer])) {
                ErrInd = ind[Pointer];
        ...
        internal ScriptLineElement CreateForLoop(ref int Pointer, string[] lines, uint[] ind, ref uint ErrInd, ref string ErrorMsg) {
            ...
            bool success = this_for.Parse(lines[Pointer]);
            if (!success) {
                ErrInd = ind[Pointer];
    ...
    internal class DataTypeObject {
        ...
        public object Value {
            get {
                switch (VarType) {
                    case var @case when @case == DataType.Integer: {
                            return Utilities.Bytes.ToInt32(InternalData);
                        }
                    case var case1 when case1 == DataType.UInteger: {
                            return Utilities.Bytes.ToUInt32(InternalData);
                        }
                    case var case2 when case2 == DataType.String: {
                            return Utilities.Bytes.ToChrString(InternalData);
                        }
                    case var case3 when case3 == DataType.Bool: {
                            if (InternalData[0] == 1) {
                                return true;
                    ...
                    case var case4 when case4 == DataType.Data: {
                            return InternalData;
                        }
                    default: {
                            return null;
            ...
            set {
                switch (VarType) {
                    case var @case when @case == DataType.Integer: {
                            InternalData = Utilities.Bytes.FromInt32((int)value);
                            break;
                        }
                    case var case1 when case1 == DataType.UInteger: {
                            InternalData = Utilities.Bytes.FromUInt32((uint)value);
                            break;
                        }
                    case var case2 when case2 == DataType.String: {
                            InternalData = Utilities.Bytes.FromChrString((string)value);
                            break;
                        }
                    case var case3 when case3 == DataType.Bool: {
        ...
        internal class ScriptElement : ScriptLineElement {
            ...
            internal bool Parse(string to_parse, bool parse_target) {
                to_parse = to_parse.Trim();
                if (parse_target)
                    LoadTarget(ref to_parse);
                OPERLIST = new ScriptElementOperand(MyParent, to_parse);
                if (!OPERLIST.ERROR_MSG.Equals("")) {
                    ERROR_MSG = OPERLIST.ERROR_MSG;
                    return false;
                }
            ...
            private void LoadTarget(ref string to_parse) {
                string str_out = "";
                    ...
                    if (to_parse.Length - i > 2 && to_parse.Substring(i, 2) == "==") { // This is a compare
                        ...
                        if (!string.IsNullOrEmpty(arg)) {
                            if (Information.IsNumeric(arg)) {
                                TARGET_INDEX = Int32.Parse(arg); // Fixed INDEX
                                ...
                                ERROR_MSG = "Target index must be able to evaluate to an integer";
                            }
                        }
                        return;
                    } else if (to_parse.Substring(i, 1) == ".") {
            ...
            internal ScriptVariable Compile(ref ExitMode exit_task) {
                var sv = OPERLIST.CompileToVariable(ref exit_task);
                ERROR_MSG = OPERLIST.ERROR_MSG;
                return sv;
            }
        ...
        internal class ScriptLoop : ScriptLineElement {
            ...
            internal bool Parse(string input) {
                try {
                    if (input.ToUpper().StartsWith("FOR "))
                        input = input.Substring(4).Trim();
                    if (input.StartsWith("(") && input.EndsWith(")")) {
                        input = input.Substring(1, input.Length - 2);
                        VAR_NAME = Tools.FeedWord(ref input, new[] { "=" });
                        if (string.IsNullOrEmpty(VAR_NAME))
                            return false;
                        input = input.Substring(1).Trim();
                        string first_part = Tools.FeedElement(ref input);
                        input = input.Trim();
                        if (string.IsNullOrEmpty(input))
                            return false; // More info needed
                        string to_part = Tools.FeedElement(ref input);
                        input = input.Trim();
                        if (string.IsNullOrEmpty(input))
                            return false; // More info needed
                        if (!(to_part.ToUpper() == "TO"))
                            return false;
                        LOOPSTART_OPER = new ScriptElementOperand(MyParent, first_part);
                        LOOPSTOP_OPER = new ScriptElementOperand(MyParent, input);
                        if (!string.IsNullOrEmpty(LOOPSTART_OPER.ERROR_MSG))
                            return false;
                        if (!string.IsNullOrEmpty(LOOPSTOP_OPER.ERROR_MSG))
                            return false;
                        return true;
                    } else {
                        return false;
            ...
            internal bool Evaluate() {
                try {
                    ...
                    if (!(sv2.Data.VarType == DataType.UInteger))
                        return false;
                    START_IND = (uint)(sv1.Value);
                    END_IND = (uint)(sv2.Value);
                    return true;
                } catch {
                    return false;
                }
        ...
        internal class ScriptReturn : ScriptLineElement {
            ...
            internal bool Parse(string to_parse) {
                to_parse = to_parse.Trim();
                OPERLIST = new ScriptElementOperand(MyParent, to_parse);
                if (!string.IsNullOrEmpty(OPERLIST.ERROR_MSG)) {
                    ERROR_MSG = OPERLIST.ERROR_MSG;
                    return false;
                }
        ...
        internal static class Tools {
            ...
            internal static string FeedParameter(ref string input) {
                ...
                for (int i = 0, loopTo = input.Length - 1; i <= loopTo; i++) {
                    counter += 1;
                    char c = char.Parse(input.Substring(i, 1));
                    strout += c;
                    if (is_in_string && c == '\"') {
                        is_in_string = false;
                    } else if (c == '\"') {
                ...
                input = input.Substring(counter).TrimStart();
                return strout.Substring(1, strout.Length - 2).Trim();
            }
            ...
            internal static ScriptVariable CompileSVars(ScriptVariable var1, ScriptVariable var2, OperandOper oper, ref string error_reason) {
                try {
                    if (oper == OperandOper.AND | oper == OperandOper.OR) {
                        if (!(var1.Data.VarType == DataType.Bool)) {
                            error_reason = "OR / AND bitwise operators only valid for Bool data types";
                            return null;
                        }
                    }
                    switch (oper) {
                                ...
                                switch (var1.Data.VarType) {
                                    case DataType.Integer: {
                                            if (var2.Data.VarType == DataType.Integer) {
                                                return new ScriptVariable("RESULT", DataType.Integer, (int)(var1.Value) + (int)(var2.Value));
                                            } else if (var2.Data.VarType == DataType.UInteger) {
                                                return new ScriptVariable("RESULT", DataType.Integer, (int)(var1.Value) + (uint)(var2.Value));
                                            } else {
                                                error_reason = "Operand data type mismatch";
                                                return null;
                                            }
                                        }
                                    case DataType.UInteger: {
                                            if (var2.Data.VarType == DataType.UInteger) {
                                                return new ScriptVariable("RESULT", DataType.UInteger, (uint)(var1.Value) + (uint)(var2.Value));
                                            } else if (var2.Data.VarType == DataType.UInteger) {
                                                return new ScriptVariable("RESULT", DataType.UInteger, (uint)(var1.Value) + (int)(var2.Value));
                                            } else {
                                                error_reason = "Operand data type mismatch";
                                                return null;
                                            }
                                    ...
                                    case DataType.String: {
                                            ...
                                            if (var2.Data.VarType == DataType.UInteger) {
                                                new_result.Value = (string)(var1.Value) + ((uint)var2.Value).ToString();
                                            } else if (var2.Data.VarType == DataType.String) {
                                                new_result.Value = (string)var1.Value + (string)var2.Value;
                                            } else {
                                                error_reason = "Operand data type mismatch";
                                                return null;
                                            }
                                    ...
                                    case DataType.Bool: {
                                            error_reason = "Add operand not valid for Bool data type";
                                            return null;
                                        }
                                }
                                break;
                                ...
                                switch (var1.Data.VarType) {
                                    ...
                                    default: {
                                            ...
                                            if (!(var1.Data.VarType == DataType.UInteger)) {
                                                error_reason = "Subtract operand only valid for Integer data type";
                                                return null;
                                            }
                                            new_result.Value = (uint)var1.Value - (uint)var2.Value;
                                            return new_result;
                                        }
                                }
                            }
                        case OperandOper.DIV: {
                                ...
                                if (var1.Data.VarType == DataType.Integer) {
                                    ...
                                    new_result.Value = (double)var1.Value / (double)var2.Value;
                                    return new_result;
                                } else if (var2.Data.VarType == DataType.UInteger) {
                                    var new_result = new ScriptVariable("RESULT", DataType.UInteger);
                                    new_result.Value = (double)var1.Value / (double)var2.Value;
                                    ...
                                    error_reason = "Division operand only valid for Integer or UInteger data type";
                                    return null;
                                }
                            }
                        case OperandOper.MULT: {
                                ...
                                if (var1.Data.VarType == DataType.Integer) {
                                    ...
                                    error_reason = "Multiply operand only valid for Integer or UInteger data type";
                                    return null;
                                }
                            }
                        case OperandOper.S_LEFT: {
                                ...
                                if (!(var1.Data.VarType == DataType.Integer)) {
                                    error_reason = "Shift-left operand only valid for Integer data type";
                                    return null;
                                }
                                int shift_value = (int)var2.Value;
                                if (shift_value > 31L) {
                                    error_reason = "Shift-left value is greater than 31-bits";
                                    return null;
                                }
                                new_result.Value = (uint)var1.Value << shift_value;
                                return new_result;
                            }
                        case OperandOper.S_RIGHT: {
                                var new_result = new ScriptVariable("RESULT", DataType.Integer);
                                ...
                                if (!(var1.Data.VarType == DataType.Integer)) {
                                    error_reason = "Shift-right operand only valid for Integer data type";
                                    return null;
                                }
                                int shift_value = (int)var2.Value;
                                if (shift_value > 31L) {
                                    error_reason = "Shift-right value is greater than 31-bits";
                                    return null;
                                }
                                new_result.Value = (uint)var1.Value >> shift_value;
                                return new_result;
                        ...
                        case OperandOper.AND: { // We already checked to make sure these are BOOL
                                ...
                                new_result.Value = (bool)var1.Value & (bool)var2.Value;
                                return new_result;
                            }
                        ...
                        case OperandOper.LESS_THAN: { // Boolean compare operators 
                                ...
                                if (var1.Data.VarType == DataType.Integer) {
                                    new_result.Value = (int)var1.Value < (int)var2.Value;
                                } else if (var1.Data.VarType == DataType.UInteger) {
                                    new_result.Value = (uint)var1.Value < (uint)var2.Value;
                                } else {
                                    error_reason = "Less-than compare operand only valid for Integer/UInteger data type";
                                    return null;
                                }
                                return new_result;
                            }
                        case OperandOper.GRT_THAN: { // Boolean compare operators
                                ...
                                if (var1.Data.VarType == DataType.Integer) {
                                    ...
                                    error_reason = "Greater-than compare operand only valid for Integer/UInteger data type";
                                    return null;
                                }
                                return new_result;
                            }
