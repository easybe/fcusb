...
namespace SPI {
    public class SPI_Programmer : MemoryDeviceUSB {
        ...
        public SPI_Programmer(FCUSB_DEVICE parent_if) {
            FCUSB = parent_if;
        }
        ...
        public bool DeviceInit() {
            ...
            this.SPIBUS_WriteRead(new byte[] { 0xC2, 0 }, ReadBuffer: ref argReadBuffer); // always select die 0
            ...
            if (ReadSuccess) {
                PrintConsole?.Invoke("Successfully opened device in SPI mode");
                ...
                PrintConsole?.Invoke(string.Format("Connected to SPI Flash (RDID:{0} REMS:{1})", RDID_Str, REMS_Str));
                MyFlashDevice = (SPI_NOR)MainApp.FlashDatabase.FindDevice(MFG, ID1, ID2, MemoryType.SERIAL_NOR, DEVICEID.FMY);
                if (MyFlashDevice is object) {
                    ...
                    PrintConsole?.Invoke(string.Format("Flash detected: {0} ({1} bytes)", this.DeviceName, this.DeviceSize.ToString("N0")));
                    PrintConsole?.Invoke(string.Format("Page erase size: {0} bytes", MyFlashDevice.ERASE_SIZE.ToString("N0")));
                    Utilities.Sleep(50);
                    return true;
                } else {
                    MyFlashDevice = new SPI_NOR("Unknown", VCC_IF.SERIAL_3V, 0U, DEVICEID.MANU, ID1);
        ...
        public byte[] ReadData(long flash_offset, long data_count) {
            if (W25M121AV_Mode) {
                byte[] argReadBuffer = null;
                this.SPIBUS_WriteRead(new byte[] { 0xC2, 0 }, ReadBuffer: ref argReadBuffer);
                WaitUntilReady();
            }
            ...
            byte dummy_clocks = 0;
            if (MainApp.MySettings.SPI_FASTREAD && FCUSB.HasLogic) {
            ...
            if (MyFlashDevice.ProgramMode == SPI_ProgramMode.Atmel45Series) {
                ...
                if ((long)MyFlashDevice.ADDRESSBITS == 8L) { // Used on ST M95010 - M95040 (8bit) and ATMEL devices (AT25010A - AT25040A)
                    ...
                    FCUSB.USB_SETUP_BULKIN(USBREQ.SPI_READFLASH, setup_class.ToBytes(), ref data_to_read, 0U);
                } else {
                ...
                if ((long)MyFlashDevice.STACKED_DIES > 1L) {
                    while (bytes_left != 0L) {
                        ...
                        FCUSB.USB_SETUP_BULKIN(USBREQ.SPI_READFLASH, setup_class.ToBytes(), ref die_data, 0U);
        ...
        public bool EraseDevice() {
            var msg = "Erasing entire flash device, total size: {0} bytes (this may take a moment)";
            PrintConsole?.Invoke(string.Format(msg, DeviceSize.ToString("N0")));
            ...
            if (MyFlashDevice.ProgramMode == SPI_ProgramMode.Atmel45Series) {
                byte[] argReadBuffer1 = null;
                this.SPIBUS_WriteRead(new byte[] { 0xC7, 0x94, 0x80, 0x9A }, ref argReadBuffer1); // Chip erase command
                Utilities.Sleep(100);
                WaitUntilReady();
            } else if (MyFlashDevice.ProgramMode == SPI_ProgramMode.SPI_EEPROM) {
                ...
                nord_timer.Start();
                var msg2 = "Erasing entire flash device, total size: {0} bytes (this may take a moment)";
                PrintConsole?.Invoke(string.Format(msg2, DeviceSize.ToString("N0")));
                int TotalPages = (int)((double)MyFlashDevice.FLASH_SIZE / (double)MyFlashDevice.PAGE_SIZE);
                for (byte i = 0, loopTo1 = (byte)(TotalPages - 1); i <= loopTo1; i++) {
                    SPIBUS_WriteEnable();
                    Utilities.Sleep(50);
                    SPIBUS_WriteData(new byte[] { MyFlashDevice.OP_COMMANDS.SE, i });
                    WaitUntilReady();
                }
            } else {
        ...
        public void WaitUntilReady() {
            try {
                ...
                if (MyFlashDevice.ProgramMode == SPI_ProgramMode.Atmel45Series) {
                    ...
                    if (MyFlashDevice is object && MyFlashDevice.ProgramMode == SPI_ProgramMode.Nordic) {
                        Utilities.Sleep(50);
                    }
                }
            } catch {
        ...
        private void LoadDeviceConfigurations() {
            ...
            if ((long)MyFlashDevice.STACKED_DIES > 1L) { // Multi-die support
                ...
                byte[] argReadBuffer1 = null;
                this.SPIBUS_WriteRead(new byte[] { MyFlashDevice.OP_COMMANDS.DIESEL, 0 }, ReadBuffer: ref argReadBuffer1);
                WaitUntilReady(); // We need to make sure DIE 0 is selected
                DIE_SELECTED = 0;
            } else if (MyFlashDevice.ProgramMode == SPI_ProgramMode.Atmel45Series) {
                byte[] argReadBuffer2 = null;
                this.SPIBUS_WriteRead(new byte[] { 0x3D, 0x2A, 0x7F, 0x9A }, ref argReadBuffer2); // Disable sector protection
            } else {
                if (MyFlashDevice.SEND_EN4B)
        ...
        private SPI_IDENT ReadDeviceID() {
            ...
            var rdid = new byte[6];
            this.SPIBUS_WriteRead(new byte[] { SPI_CmdDef.RDID }, ref rdid); // This reads SPI CHIP ID
            var rems = new byte[2];
            this.SPIBUS_WriteRead(new byte[] { SPI_CmdDef.REMS, 0, 0, 0 }, ref rems); // Some devices (such as SST25VF512) only support REMS
            var res = new byte[1];
            this.SPIBUS_WriteRead(new byte[] { SPI_CmdDef.RES, 0, 0, 0 }, ref res);
            DEVICEID.MANU = rdid[0];
            DEVICEID.RDID = (uint)rdid[1] << 24 | (uint)rdid[2] << 16 | (uint)rdid[3] << 8 | rdid[4];
            DEVICEID.FMY = rdid[5];
            DEVICEID.REMS = (ushort)(rems[0] << 8 | rems[1]);
            DEVICEID.RES = res[0];
            return DEVICEID;
        }
        ...
        private void ReadFlagStatusRegister() {
            ...
            var flag = new byte[] { 0 };
            do
                SPIBUS_WriteRead(new byte[] { MyFlashDevice.OP_COMMANDS.RDFR }, ref flag);
            while (((flag[0] >> 7) & 1) != 1);
        }
        ...
        public void ResetDevice() {
            byte[] argReadBuffer = null;
            this.SPIBUS_WriteRead(new byte[] { 0xF0 }, ReadBuffer: ref argReadBuffer); // SPI RESET COMMAND
            // Other commands: 0x66 and 0x99
            Utilities.Sleep(10);
        }

        private void LoadVendorSpecificConfigurations() {
            if (MyFlashDevice.ProgramMode == SPI_ProgramMode.Atmel45Series) { // May need to load the current page mode
            ...
            if ((int)MyFlashDevice.MFG_CODE == 0x34) { // Cypress MFG ID
                ...
                if (SEMPER_SPI) {
                    SPIBUS_WriteEnable();
                    byte[] argReadBuffer1 = null;
                    this.SPIBUS_WriteRead(new byte[] { MyFlashDevice.OP_COMMANDS.EWSR }, ReadBuffer: ref argReadBuffer1);
                    byte[] argReadBuffer2 = null;
                    this.SPIBUS_WriteRead(new byte[] { MyFlashDevice.OP_COMMANDS.WRSR, 0, 0, 0x88, 0x18 }, ReadBuffer: ref argReadBuffer2); // Set sector to uniform 256KB / 512B Page size
                }
                ...
                if (SEMPER_SPI_HF) {
                    SPIBUS_WriteEnable();
                    byte[] argReadBuffer3 = null;
                    this.SPIBUS_WriteRead(new byte[] { 0x71, 0x80, 0, 4, 0x18 }, ReadBuffer: ref argReadBuffer3); // Enables 512-byte buffer
                    SPIBUS_WriteEnable();
                    byte[] argReadBuffer4 = null;
                    this.SPIBUS_WriteRead(new byte[] { 0x71, 0x80, 0, 3, 0x80 }, ReadBuffer: ref argReadBuffer4); // Enables 4-byte mode
                }
            }
        }
        ...
        public bool AT45_SetPageConfiguration(bool EnableExtPage) {
            if (EnableExtPage) {
                byte[] argReadBuffer = null;
                this.SPIBUS_WriteRead(new byte[] { 0x3D, 0x2A, 0x80, 0xA7 }, ref argReadBuffer);
            } else {
                byte[] argReadBuffer1 = null;
                this.SPIBUS_WriteRead(new byte[] { 0x3D, 0x2A, 0x80, 0xA6 }, ref argReadBuffer1);
        ...
        private byte[] AT45_ReadData(uint flash_offset, uint data_count) {
            ...
            var addr_bytes = Utilities.Bytes.FromUInt24((PageAddr << AddrOffset) + PageOffset);
            ...
            FCUSB.USB_SETUP_BULKIN(USBREQ.SPI_READFLASH, setup_class.ToBytes(), ref data_out, 0U);
            return data_out;
        }
        ...
        private bool AT45_EraseSector(uint flash_offset) {
            ...
            var addrbytes = Utilities.Bytes.FromUInt24(blocknum << (int)(AddrOffset + 3L));
            byte[] argReadBuffer = null;
        ...
        private bool AT45_WriteData(uint offset, byte[] DataOut) {
            try {
                ...
                while (BytesLeft != 0L) {
                    ...
                    var PageCmd = Utilities.Bytes.FromUInt24(PageAddr << AddrOffset);
            ...
            try {
                ...
                if (FCUSB.HasLogic) {
                    ...
                    for (int i = 0, loopTo = Loops - 1; i <= loopTo; i++) {
                        ...
                        result = FCUSB.USB_SETUP_BULKOUT(USBREQ.SPI_WRITEFLASH, setup_packet.ToBytes(), data, 0U, 1000);
                        if (!result)
                            return false;
                        Utilities.Sleep(5);
                    ...
                    result = FCUSB.USB_SETUP_BULKOUT(USBREQ.SPI_WRITEFLASH, setup_packet.ToBytes(), data_out, 0U);
                }
        ...
        private bool WriteData_AAI(uint flash_offset, byte[] data, bool word_mode) {
            ...
            uint ctrl = (uint)(Utilities.BoolToInt(word_mode) + 1);
            bool Result = FCUSB.USB_SETUP_BULKOUT(USBREQ.SPI_WRITEDATA_AAI, setup_packet.ToBytes(), data, ctrl);
            if (!Result)
                return false;
        ...
        public void SPIBUS_Setup(SPI_SPEED bus_speed) {
            uint clock_speed = (uint)MainApp.GetMaxSpiClock(FCUSB.HWBOARD, bus_speed);
            FCUSB.USB_SPI_INIT((uint)MainApp.MySettings.SPI_MODE, clock_speed);
        ...
        private bool SPIBUS_WriteData(byte[] DataOut) {
            ...
            if (!Success) { PrintConsole?.Invoke("SPI: Error writing data to USB port"); }
            return true;
        }
        ...
        private bool SPIBUS_ReadData(ref byte[] Data_In) {
            ...
            if (!Success) { PrintConsole?.Invoke("SPI: Error reading data from USB port"); }
            return Success;
        }
        ...
        public void SetProgPin(bool enabled) {
            try {
                ...
                FCUSB.USB_CONTROL_MSG_OUT(USBREQ.SPI_PROG, null, value);
            } catch {
            }
        }
    ...
    internal class WriteSetupPacket {
        ...
        public SPI_QUAD_SUPPORT SPI_MODE { get; set; } = SPI_QUAD_SUPPORT.NO_QUAD;

        public WriteSetupPacket(SPI_NOR spi_dev, uint offset, uint d_count) {
