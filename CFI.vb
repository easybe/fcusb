...
#Region "Sector / Addresses"
    ...
    Public Sub EraseSector(ByVal Sector As Integer)
        Monitor.Enter(EJ_IF.ClientLock)
        Try
    ...
    Private Sub amd_erasewait(ByVal SectorOffset As Integer, Optional AllowTimeout As Boolean = True)
        Try
            ...
            Do Until mydata = &HFFFFFFFFL
                Utilities.Sleep(100)
                mydata = EJ_IF.Memory_Read_W(CUInt(MyDeviceBase + SectorOffset))
...
#End Region
    ...
    Private MyDeviceSize As UInt32 'Size in number of bytes
    ...
    Public Function DetectFlash(ByVal BaseAddress As UInt32) As Boolean
        ...
        If Enable_JEDEC_Mode() Then
            ...
            If MyDeviceMode = DeviceAlgorithm.NotDefined Then 'Possible non-cfi device
                Dim FirstWord As UInt32 = EJ_IF.Memory_Read_W(MyDeviceBase)
                FirstWord = EJ_IF.Memory_Read_W(MyDeviceBase) 'Read this twice for some unknown reason
                ...
                EJ_IF.SetParameter(5, BaseAddress) 'Legacy devices require flash commands, this sets the flash base address
            Else
                MyDeviceID.ManufactureID = CByte(EJ_IF.Memory_Read_H(MyDeviceBase) And &HFF)
                If MyDeviceMode = DeviceAlgorithm.Intel Or MyDeviceMode = DeviceAlgorithm.Intel_Sharp Then
                    MyDeviceID.PartNumber = EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H2))
                ElseIf MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu Or MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu_Extended Then
                    MyDeviceID.PartNumber = EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H22))
                ElseIf MyDeviceMode = DeviceAlgorithm.SST Then
                    MyDeviceID.PartNumber = EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H2))
                    EJ_IF.SetParameter(5, BaseAddress) 'SST flashes require flash commands, this sets the flash base address
                End If
                If MyDeviceID.ManufactureID = 1 And MyDeviceID.PartNumber = &H227E Then 'Updates the full PartNumber for SPANSION devices
                    Dim cycle_one As Byte = CByte(&HFF And EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + +&H2)))
                    Dim cycle_two As Byte = CByte(&HFF And EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + +&H1C)))
                    Dim cycle_thr As Byte = CByte(&HFF And EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + +&H1E)))
                ...
                WriteConsole(String.Format(RM.GetString("fcusb_cfi_founddev"), BaseStr)) '"Loaded CFI compatible flash device at: {0}"
            End If
            LoadFlashName()
            WriteConsole(String.Format(RM.GetString("fcusb_cfi_jedecid"), MyDeviceID.ToString))
            WriteConsole(String.Format(RM.GetString("fcusb_cfi_base"), "0x" & Hex(MyDeviceBase).PadLeft(8, "0")))
            WriteConsole(String.Format(RM.GetString("fcusb_cfi_desc"), CStr(MyDeviceName & " (" & Format(MyDeviceSize, "#,###") & " bytes)")))
            PrintProgrammingMode() '"Programming mode: etc"
        Else
            WriteConsole(RM.GetString("fcusb_cfi_err1"))
            Return False
        End If
        Return True
    End Function
    ...
    Private Sub LoadFlashName()
        Dim flash As FlashMemory.Device = FlashDatabase.FindDevice(MyDeviceID.ManufactureID, MyDeviceID.PartNumber, FlashMemory.MemoryType.JTAG)
        If flash IsNot Nothing Then
            MyDeviceName = flash.NAME
        Else
    ...
    Private Sub PrintProgrammingMode()
        ...
        End Select
        WriteConsole(RM.GetString("fcusb_cfi_pmode") & ": " & AlgStr & " " & InterfaceStr & " " & BusWidthString)
    End Sub

    Private Sub write_command(ByVal offset As UInt32, ByVal data As UInt32)
        Select Case MyDeviceBus
            Case DeviceBus.X8
                EJ_IF.Memory_Write_B(CUInt(MyDeviceBase + offset), data)
            Case DeviceBus.X16
                EJ_IF.Memory_Write_H(CUInt(MyDeviceBase + offset), data)
            Case DeviceBus.X32
                EJ_IF.Memory_Write_W(CUInt(MyDeviceBase + offset), data)
        End Select
    End Sub
    ...
    Private Function Enable_CFI_Mode(ByVal BusMode As DeviceBus) As Boolean
        Select Case BusMode
            Case DeviceBus.X8
                EJ_IF.Memory_Write_B(CUInt(MyDeviceBase + &HAA), &H98) 'CFI Mode Command
            Case DeviceBus.X16
                EJ_IF.Memory_Write_H(CUInt(MyDeviceBase + &HAA), &H98) 'CFI Mode Command
            Case DeviceBus.X32
                EJ_IF.Memory_Write_W(CUInt(MyDeviceBase + &HAA), &H98) 'CFI Mode Command 
        End Select
        Utilities.Sleep(50) 'If the command succeded, we need to wait for the device to switch modes
        Dim ReadBack As UInt32 = CUInt(EJ_IF.Memory_Read_H(MyDeviceBase + &H20UI))
        ReadBack = CUInt((ReadBack << 8) + EJ_IF.Memory_Read_H(MyDeviceBase + &H22UI))
        ReadBack = CUInt((ReadBack << 8) + EJ_IF.Memory_Read_H(MyDeviceBase + &H24UI))
    ...
    Private Function Enable_CFI_Mode_ForSST()
        EJ_IF.Memory_Write_B(MyDeviceBase + &HAAAA, &HAA)
        EJ_IF.Memory_Write_B(MyDeviceBase + &H5554, &H55)
        EJ_IF.Memory_Write_B(MyDeviceBase + &HAAAA, &H98)
    ...
    Private Sub Load_CFI_Data()
        MyDeviceSize = CInt(2 ^ EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H4E)))
        Dim DeviceCommandSet As UShort = CUShort(&HFF And EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H26))) << 8
        DeviceCommandSet += CByte(&HFF And EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H28)))
        MyDeviceMode = DeviceCommandSet
        MyDeviceInterface = CInt(EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H50)))
        Flash_BlockCount = EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H58))
        Dim BootFlag As UInt32 = EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + &H9E))
        ReDim Flash_EraseBlock(Flash_BlockCount - 1)
        ReDim Flash_EraseSize(Flash_BlockCount - 1)
        Dim BlockAddress As UInt32 = &H5A 'Start address of block 1 information
        For i = 1 To Flash_BlockCount
            Flash_EraseBlock(i - 1) = ((EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + BlockAddress + 2)) << 8) + EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + BlockAddress))) + 1
            Flash_EraseSize(i - 1) = ((EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + BlockAddress + 6)) << 8) + EJ_IF.Memory_Read_H(CUInt(MyDeviceBase + BlockAddress + 4))) * 256
    ...
    Public Sub Read_Mode()
        If MyDeviceMode = DeviceAlgorithm.NotDefined Then
            EJ_IF.Memory_Write_B(MyDeviceBase, &HFF) 'For Intel / Sharp
            EJ_IF.Memory_Write_B(MyDeviceBase, &H50)
            EJ_IF.Memory_Write_B(MyDeviceBase + &HAAA, &HAA) 'For AMD
            EJ_IF.Memory_Write_B(MyDeviceBase + &H555, &H55)
            EJ_IF.Memory_Write_B(MyDeviceBase + &HAAAA, &HAA) 'For SST
            EJ_IF.Memory_Write_B(MyDeviceBase + &H5554, &H55)
            EJ_IF.Memory_Write_B(MyDeviceBase + &HAAAA, &HF0)
            EJ_IF.Memory_Write_B(MyDeviceBase, &HF0) 'For LEGACY
        ElseIf MyDeviceMode = DeviceAlgorithm.Intel Or MyDeviceMode = DeviceAlgorithm.Intel_Sharp Then
            write_command(0, &HFF)
            write_command(0, &H50)
        ElseIf MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu Or MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu_Extended Then
    ...
    Public Sub WaitUntilReady()
        ...
        If MyDeviceMode = DeviceAlgorithm.Intel Or MyDeviceMode = DeviceAlgorithm.Intel_Sharp Then
            Do
                ...
                sr = EJ_IF.Memory_Read_H(MyDeviceBase)
    ...
    Public Function EraseBulk() As Boolean
        WriteConsole(RM.GetString("fcusb_cfi_erasing"))
        If MyDeviceMode = DeviceAlgorithm.Intel Or MyDeviceMode = DeviceAlgorithm.Intel_Sharp Then
        ...
        End If
        Read_Mode()
        WriteConsole(RM.GetString("fcusb_cfi_erasecomplete"))
        Return True
    End Function
    ...
    Public Function ReadData(ByVal Offset As UInt32, ByVal NumberOfBytes As UInt32) As Byte()
        ...
        Try
            If UseBulkRead Then 'This is significantly faster
                DataOut = EJ_IF.Memory_Read_Bulk(MyDeviceBase + Offset, NumberOfBytes)
            Else
                Dim c As Integer = 0
                ReDim DataOut(NumberOfBytes - 1)
                For i = 0 To (NumberOfBytes / 4) - 1
                    Dim word As UInt32 = EJ_IF.Memory_Read_W(MyDeviceBase + Offset + (i * 4))
    ...
    Public Sub WriteData(ByVal Offset As UInt32, ByVal Data() As Byte)
        Monitor.Enter(EJ_IF.ClientLock) 'Prevents multiple threads from accessing the bus while we are doing a sequence of commands
        Try
            If MyDeviceMode = DeviceAlgorithm.Intel Or MyDeviceMode = DeviceAlgorithm.Intel_Sharp Then
                If EJ_IF.SUPPORT_INTELFLASH And (Not EJ_IF.TargetDevice.NoDMA) Then 'Our fast method only works for DMA enabled targets
                    EJ_IF.dmaWriteFlash(MyDeviceBase + Offset, Data, EJTAG.cfi_mode.Intel_16)
                Else
                    For i = 0 To (Data.Length - 1) Step 4 'We will write data 4 bytes at a time
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i, &H40)
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i, (CInt(Data(i + 1)) << 8) + Data(i + 0))
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i + 2, &H40)
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i + 2, (CInt(Data(i + 3)) << 8) + Data(i + 2))
                    Next
                End If
                Read_Mode()
            ElseIf MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu Or MyDeviceMode = DeviceAlgorithm.AMD_Fujitsu_Extended Then
                ...
                If (EJ_IF.SUPPORT_AMDFLASH And (Not EJ_IF.TargetDevice.NoDMA)) Then 'Our fast method only works for DMA enabled targets
                    EJ_IF.dmaWriteFlash(MyDeviceBase + Offset, Data, EJTAG.cfi_mode.AMD_16)
                Else
                    For i = 0 To (Data.Length - 1) Step 4 'We will write data 4 bytes at a time
                        EJ_IF.Memory_Write_H(MyDeviceBase, &HA0)
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i, (CInt(Data(i + 1)) << 8) + Data(i + 0))
                        EJ_IF.Memory_Write_H(MyDeviceBase, &HA0)
                        EJ_IF.Memory_Write_H(MyDeviceBase + Offset + i + 2, (CInt(Data(i + 3)) << 8) + Data(i + 2))
                    Next
                ...
                write_command(0, &H0)
            ElseIf MyDeviceMode = DeviceAlgorithm.AMD_NoBypass Then
                If EJ_IF.SUPPORT_LEGACYFLASH And (Not EJ_IF.TargetDevice.NoDMA) Then 'Our fast method only works for DMA enabled targets
                    EJ_IF.dmaWriteFlash(MyDeviceBase + Offset, Data, EJTAG.cfi_mode.NoBypass)
                Else
                    For i = 0 To (Data.Length - 1) Step 4 'We will write data 4 bytes at a time
                        write_command(&HAAA, &HAA)
                        write_command(&H555, &H55)
                ...
                'write_command(0, &HF0)
            ElseIf MyDeviceMode = DeviceAlgorithm.SST Then
                If EJ_IF.SUPPORT_LEGACYFLASH And (Not EJ_IF.TargetDevice.NoDMA) Then
                    EJ_IF.dmaWriteFlash(MyDeviceBase + Offset, Data, EJTAG.cfi_mode.SST)
                Else
                    For i = 0 To (Data.Length - 1) Step 4 'We will write data 4 bytes at a time
                        write_command(&HAAAA, &HAA)
                        write_command(&H5554, &H55)
    ...
    Public Function ReadByte(ByVal offset As UInt32) As Byte
        Return EJ_IF.Memory_Read_B(MyDeviceBase + offset)
    End Function
