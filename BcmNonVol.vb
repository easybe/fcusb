...
Public Class BcmNonVol
    ...
    Public Sub WriteConfig()
        ...
        FCUSB_FlashBlindly(GetCfgArea, NonVolCfg.GetConfig, flashIndex)
    ...
    Public Function GetFirmwareName() As String
        Try
            ...
            Dim header() As Byte = FCUSB_ReadMemory(HeaderLocation, 92, flashIndex, Nothing)
    ...
    Public Function GetFirmwareLen() As Integer
        Dim header() As Byte = FCUSB_ReadMemory(CUInt(GetEndAdr() + 1), 92, flashIndex, Nothing)
        Dim HeaderCls As New BcmHeaderClass(header)
        Return HeaderCls.FwLen
    End Function

    Private Function GetCfg(ByVal Area As Integer) As Byte()
        Dim Word() As Byte = FCUSB_ReadMemory(CUInt(Area), 4, flashIndex, Nothing)
        Dim LenStr As String = Utilities.Bytes.ToHexString(Word)
        Dim LenInt As Integer = Utilities.HexToInt(LenStr)
        If LenInt < 1 Or LenInt > 16384 Then '"Cfg Length is Invalid"
            Return Nothing
        End If
        Dim b() As Byte = FCUSB_ReadMemory(CUInt(Area), LenInt, flashIndex, Nothing)
        Return b
    End Function
    ...
    Private Function GetCfgArea() As UInteger
        ...
        If i > 0 Then
            Dim sB() As Byte = FCUSB_ReadMemory(CUInt(i), 1, flashIndex, Nothing)
            Select Case sB(0)
                Case 255 '"FF"
    ...
    Private Function GetEndAdr() As Integer
        Dim HexAdr As Integer = Utilities.HexToInt("FFF0")
        Dim ByteLine As String = Utilities.Bytes.ToHexString(FCUSB_ReadMemory(CUInt(HexAdr), 16, flashIndex, Nothing))
    ...
    Public Class Bcm3348NonVolCfg
        ...
        Private Sub ParseCfg(ByVal MyCfg() As Byte)
            MyChecksum = Hex(MyCfg(4)).PadLeft(2, CChar("0")) & Hex(MyCfg(5)).PadLeft(2, CChar("0")) & _
