Public Class NonVol_1

    Private Sub NonVol_1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub cmd_read_config_Click(sender As Object, e As EventArgs) Handles cmd_read_config.Click
        Try
            ...
            Dim sr() As Byte = SPI_IF.ReadStatusRegister(1)
            Dim cr(1) As Byte
            SPI_IF.SPIBUS_WriteRead({&HB5}, cr)
            WriteConsole("Status register: 0x" & Hex(sr(0)).PadLeft(2, "0"))
            WriteConsole("Nonvol config register: 0x" & Hex(cr(0)).PadLeft(2, "0") & Hex(cr(1)).PadLeft(2, "0"))
    ...
    Private Sub cmd_write_config_Click(sender As Object, e As EventArgs) Handles cmd_write_config.Click
        Try
            ...
            SPI_IF.SPIBUS_WriteRead({&HB1, cr(0), cr(1)})
            Dim sf(0) As Byte
            Do While ((sf(0) And &H80) = 0)
                SPI_IF.SPIBUS_WriteRead({&H70}, sf)
            Loop
            Utilities.Sleep(10)
            SPI_IF.WaitUntilReady()
            WriteConsole("Verifing the nonvolatile registers have been successfully programmed")
            Dim verify_sr() As Byte = SPI_IF.ReadStatusRegister(1)
            Dim verify_cr(1) As Byte
            SPI_IF.SPIBUS_WriteRead({&HB5}, verify_cr)
            ...
            Else
                ...
                WriteConsole("Error programming status register, wrote: " & read_str & ", and read back: " & read_str)
            End If
            If verify_cr(0) = cr(0) And verify_cr(1) = cr(1) Then
            Else
                Failed = True
                Dim wrote_str As String = "0x" & Hex(cr(0)).PadLeft(2, "0") & Hex(cr(1)).PadLeft(2, "0")
                Dim read_str As String = "0x" & Hex(verify_cr(0)).PadLeft(2, "0") & Hex(verify_cr(1)).PadLeft(2, "0")
                WriteConsole("Error programming nonvol config register, wrote: " & read_str & ", and read back: " & read_str)
            End If
            If Failed Then
                LoadCurrentConfigBits()
