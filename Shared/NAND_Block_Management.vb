Public Class NAND_Block_Management
    Private MyNand As FlashMemory.NAND_Flash
    Private SELECTED_BLOCK As Integer = -1
    Private PERFORMING_ANALYZE As Boolean = False
    Delegate Sub cbSetStatus(ByVal msg As String)
    Delegate Sub cbSetImage(ByVal img As Image)
    Delegate Sub cbSetExit(ByVal allow As Boolean)

    Private CancelAnalyze As Boolean = False

    Dim BLK_GRN As Image = My.Resources.BLOCK_GREEN
    Dim BLK_BLU As Image = My.Resources.BLOCK_BLUE
    Dim BLK_BLK As Image = My.Resources.BLOCK_BLACK
    Dim BLK_RED As Image = My.Resources.BLOCK_RED
    Dim BLK_CHK As Image = My.Resources.BLOCK_CHK
    Dim BLK_UNK As Image = My.Resources.BLOCK_MARIO

    Private Sub NAND_Block_Management_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Me.MinimumSize = Me.Size
        Me.MaximumSize = New Point(Me.Size.Width, 5000)

        MyNand = EXT_IF.MyFlashDevice
        Me.Text = "NAND Block Management (" & MyNand.NAME & ")"
        Dim TotalBlocks As UInt32 = MyNand.SECTOR_COUNT 'includes bad blocks
        Dim TotalRowsNeeded As Integer = TotalBlocks / 32
        Me.lbl_desc.Text = "NAND Block Map (" & Format(TotalBlocks, "#,###") & " total blocks; " & Format(MyNand.BLOCK_SIZE, "#,###") & " bytes each)"
        BlockMap.Width = 600
        BlockMap.Height = (TotalRowsNeeded * 14) + 8
        DrawImage()
    End Sub

    Private Declare Function ShowScrollBar Lib "user32.dll" (ByVal hWnd As IntPtr, ByVal wBar As Integer, ByVal bShow As Boolean) As Boolean

    Protected Overrides Sub WndProc(ByRef m As System.Windows.Forms.Message)
        Try
            ShowScrollBar(Panel1.Handle, 0, False)
        Catch ex As Exception
        End Try
        MyBase.WndProc(m)
    End Sub

    Private Sub DrawBlockImage(ByVal x As Integer, ByVal y As Integer, ByVal status As ExtPort.NAND_BLOCK_STATUS, ByRef g As Graphics)
        Try
            Dim offset As Integer = 56
            Select Case status
                Case ExtPort.NAND_BLOCK_STATUS.Valid
                    g.DrawImage(BLK_GRN, offset + (x * 14) + 4, (y * 14) + 4, 14, 14)
                Case ExtPort.NAND_BLOCK_STATUS.Bad_Marked
                    g.DrawImage(BLK_BLU, offset + (x * 14) + 4, (y * 14) + 4, 14, 14) 'Lets make this blue in the future
                Case ExtPort.NAND_BLOCK_STATUS.Bad_Manager
                    g.DrawImage(BLK_BLK, offset + (x * 14) + 4, (y * 14) + 4, 14, 14)
                Case ExtPort.NAND_BLOCK_STATUS.Bad_ByError
                    g.DrawImage(BLK_RED, offset + (x * 14) + 4, (y * 14) + 4, 14, 14)
                Case ExtPort.NAND_BLOCK_STATUS.Unknown
                    g.DrawImage(BLK_UNK, offset + (x * 14) + 4, (y * 14) + 4, 14, 14)
            End Select
        Catch ex As Exception
        End Try
    End Sub

    Private Sub cmdClose_Click(sender As Object, e As EventArgs) Handles cmdClose.Click
        Me.Close()
    End Sub

    Private Sub BlockMap_MouseMove(sender As Object, e As MouseEventArgs) Handles BlockMap.MouseMove
        If PERFORMING_ANALYZE Then Exit Sub
        Dim PreviousBlock As Integer = SELECTED_BLOCK
        If ((e.X <= 60) Or (e.Y <= 4)) Or (e.X >= 508) Then
            SELECTED_BLOCK = -1
        Else
            Dim x As Integer = Math.Floor((e.X - 60) / 14)
            Dim y As Integer = Math.Floor((e.Y - 4) / 14)
            SELECTED_BLOCK = (y * 32) + x
            If SELECTED_BLOCK > EXT_IF.NAND_MAP.Count - 1 Then SELECTED_BLOCK = -1
        End If
        If Not PreviousBlock = SELECTED_BLOCK Then
            DrawImage()
        End If
        If (SELECTED_BLOCK = -1) Then
            MyStatus.Text = ""
        Else
            Dim block_info As ExtPort.MAPPING = EXT_IF.NAND_MAP(SELECTED_BLOCK)
            Dim block_start_hexstr As String = "0x" & Hex(block_info.Physical_Start).PadLeft(8, "0")
            Dim block_end_hexstr As String = "0x" & Hex(block_info.Physical_Start + MyNand.BLOCK_SIZE - 1).PadLeft(8, "0")
            MyStatus.Text = "Selected block: " & block_start_hexstr & "-" & block_end_hexstr
            Select Case block_info.Status
                Case ExtPort.NAND_BLOCK_STATUS.Valid
                    MyStatus.Text &= " (valid)"
                Case ExtPort.NAND_BLOCK_STATUS.Bad_Marked
                    MyStatus.Text &= " (user discarded)"
                Case ExtPort.NAND_BLOCK_STATUS.Bad_Manager
                    MyStatus.Text &= " (bad marker)"
                Case ExtPort.NAND_BLOCK_STATUS.Bad_ByError
                    MyStatus.Text &= " (write error)"
            End Select
        End If
    End Sub

    Private Sub BlockMap_MouseLeave(sender As Object, e As EventArgs) Handles BlockMap.MouseLeave
        If PERFORMING_ANALYZE Then Exit Sub
        MyStatus.Text = ""
        Dim PreviousBlock As Integer = SELECTED_BLOCK
        SELECTED_BLOCK = -1
        If Not PreviousBlock = SELECTED_BLOCK Then
            DrawImage()
        End If
    End Sub

    Private Sub DrawImage()
        Try
            Dim my_bmp As Bitmap = New Bitmap(BlockMap.Width, BlockMap.Height)
            Dim gfx As Graphics = Graphics.FromImage(my_bmp)
            gfx.TextRenderingHint = Drawing.Text.TextRenderingHint.AntiAlias
            Dim x As Integer = 0
            Dim y As Integer = 0
            Dim MyFont As New Font("Lucida Console", 8)
            For i As UInt32 = 0 To MyNand.SECTOR_COUNT - 1
                Dim block_info As ExtPort.MAPPING = EXT_IF.NAND_MAP(i)
                If x = 0 Then
                    Dim hex_str As String = Hex(block_info.Physical_Start).PadLeft(8, "0")
                    gfx.DrawString(hex_str, MyFont, Brushes.Black, 0, (y * 14) + 5)
                End If
                DrawBlockImage(x, y, block_info.Status, gfx)
                If (i = SELECTED_BLOCK) Then
                    If PERFORMING_ANALYZE Then
                        gfx.DrawImage(BLK_CHK, 60 + (x * 14), (y * 14) + 4, 14, 14)
                    Else
                        gfx.DrawRectangle(Pens.Black, 60 + (x * 14), 4 + (y * 14), 13, 13)
                    End If
                End If
                x = x + 1
                If x = 32 Then
                    x = 0
                    y = y + 1
                End If
            Next
            SetImage(my_bmp)
        Catch ex As Exception
        End Try
    End Sub

    Private Sub BlockMap_DoubleClick(sender As Object, e As EventArgs) Handles BlockMap.DoubleClick
        Try
            If PERFORMING_ANALYZE Then Exit Sub
            If SELECTED_BLOCK = -1 Then Exit Sub
            Dim block_info As ExtPort.MAPPING = EXT_IF.NAND_MAP(SELECTED_BLOCK)
            If block_info.Status = ExtPort.NAND_BLOCK_STATUS.Valid Then
                block_info.Status = ExtPort.NAND_BLOCK_STATUS.Bad_Marked
            Else
                block_info.Status = ExtPort.NAND_BLOCK_STATUS.Valid
            End If
            DrawImage()
        Catch ex As Exception
        End Try
    End Sub

    Public Sub SetStatus(ByVal Msg As String)
        If Me.InvokeRequired Then
            Dim d As New cbSetStatus(AddressOf SetStatus)
            Me.Invoke(d, New Object() {[Msg]})
        Else
            MyStatus.Text = Msg
            Application.DoEvents()
        End If
    End Sub

    Public Sub SetImage(ByVal img As Image)
        If Me.InvokeRequired Then
            Dim d As New cbSetImage(AddressOf SetImage)
            Me.Invoke(d, New Object() {img})
        Else
            BlockMap.Image = img
            BlockMap.Refresh()
            Application.DoEvents()
        End If
    End Sub

    Public Sub SetExit(ByVal Allow As Boolean)
        If Me.InvokeRequired Then
            Dim d As New cbSetExit(AddressOf SetExit)
            Me.Invoke(d, New Object() {Allow})
        Else
            cmdClose.Enabled = Allow
            If Allow Then
                cmdAnalyze.Text = "Analyze"
            Else
                cmdAnalyze.Text = "Cancel"
            End If
            Me.Enabled = True
        End If
    End Sub

    Private Sub cmdAnalyze_Click(sender As Object, e As EventArgs) Handles cmdAnalyze.Click
        Me.Enabled = False
        If (cmdAnalyze.Text = "Analyze") Then
            If MsgBox("Warning, this operation will erase and overwrite all blocks, continue?", MsgBoxStyle.YesNo, "Confirm operation") = MsgBoxResult.Yes Then
                Dim td As New Threading.Thread(AddressOf AnalyzeTd)
                td.Start()
            Else
                Me.Enabled = True
            End If
        Else
            CancelAnalyze = True
            Application.DoEvents()
            Threading.Thread.Sleep(50)
        End If
    End Sub

    Private Sub AnalyzeTd()
        Dim verify_data() As Byte
        CancelAnalyze = False
        Threading.Thread.CurrentThread.Name = "AnalyzeTd"
        Dim BadBlockCounter As Integer = 0
        SetExit(False)
        PERFORMING_ANALYZE = True
        SELECTED_BLOCK = -1
        Try
            For i As UInt32 = 0 To EXT_IF.NAND_MAP.Count - 1
                EXT_IF.NAND_MAP(i).Status = ExtPort.NAND_BLOCK_STATUS.Unknown
            Next
            DrawImage()
            Dim test_data(MyNand.BLOCK_SIZE - 1) As Byte
            For i = 0 To test_data.Length - 1
                test_data(i) = ((i + 1) And 255)
            Next
            For i As UInt32 = 0 To EXT_IF.NAND_MAP.Count - 1
                If CancelAnalyze Then
                    For counter As UInt32 = i To EXT_IF.NAND_MAP.Count - 1
                        EXT_IF.NAND_MAP(counter).Status = ExtPort.NAND_BLOCK_STATUS.Valid
                    Next
                    Exit For
                End If
                Dim block_info As ExtPort.MAPPING = EXT_IF.NAND_MAP(i)
                Dim block_start_hexstr As String = "0x" & Hex(block_info.Physical_Start).PadLeft(8, "0")
                SELECTED_BLOCK = i
                DrawImage() 'Draw checkbox
                SetStatus("Verifying block: " & block_start_hexstr)
                Dim ErrorCount As Integer = 0
                Dim ValidBlock As Boolean = True
                Do 'Write block up to 3 times
                    ValidBlock = True
                    EXT_IF.NAND_ERASEBLOCK(block_info.Physical_Start, FlashMemory.FlashArea.Main, True)
                    EXT_IF.NAND_WRITEPAGE(block_info.Physical_Start, test_data, FlashMemory.FlashArea.Main)
                    Utilities.Sleep(20)
                    verify_data = EXT_IF.ReadBulk(block_info.Physical_Start, test_data.Length, FlashMemory.FlashArea.Main)
                    If Not Utilities.ArraysMatch(verify_data, test_data) Then
                        ErrorCount += 1
                        ValidBlock = False
                    End If
                    If ValidBlock Then Exit Do
                Loop While (ErrorCount < 3)
                If ValidBlock Then
                    EXT_IF.NAND_MAP(i).Status = ExtPort.NAND_BLOCK_STATUS.Valid
                Else
                    EXT_IF.NAND_MAP(i).Status = ExtPort.NAND_BLOCK_STATUS.Bad_ByError
                    BadBlockCounter += 1
                End If
            Next
        Catch ex As Exception
        Finally
            SetStatus("NAND blocks analyzed (" & BadBlockCounter & " bad blocks found)")
            SELECTED_BLOCK = -1
            PERFORMING_ANALYZE = False
            SetExit(True)
            DrawImage()
        End Try
    End Sub

End Class