...
Public Class PARALLEL_MEMORY : Implements MemoryDeviceUSB
    ...
    Public Function DeviceInit() As Boolean Implements MemoryDeviceUSB.DeviceInit
        ...
        If DetectFlashDevice() Then
            ...
            If (MyAdapter = MEM_PROTOCOL.NAND_X8_ASYNC) Then
                device_matches = FlashDatabase.FindDevices(FLASH_IDENT.MFG, FLASH_IDENT.ID1, FLASH_IDENT.ID2, MemoryType.PARALLEL_NAND)
            Else
...
#Region "Public Interface"
    ...
    Friend ReadOnly Property DeviceSize As Long Implements MemoryDeviceUSB.DeviceSize
        Get
            If MyFlashDevice.FLASH_TYPE = MemoryType.PARALLEL_NAND Then
    ...
    Public Function SectorErase(sector_index As UInt32, Optional memory_area As FlashArea = FlashArea.Main) As Boolean Implements MemoryDeviceUSB.SectorErase
        ...
        If MyFlashDevice.FLASH_TYPE = MemoryType.PARALLEL_NAND Then
            Dim page_addr As UInt32 = (DirectCast(MyFlashDevice, P_NAND).PAGE_COUNT * sector_index)
        ...
        Get
            ...
            Else
                ...
                Select Case memory_area
                    Case FlashArea.Main
                        Return (nand_dev.PAGE_COUNT * nand_dev.PAGE_SIZE)
                    Case FlashArea.OOB
                        Return (nand_dev.PAGE_COUNT * nand_dev.PAGE_EXT)
                    Case FlashArea.All
                        Return (nand_dev.PAGE_COUNT * (nand_dev.PAGE_SIZE + nand_dev.PAGE_EXT))
                End Select
                Return 0
            End If
        End Get
...
#End Region
    ...
    Private Function GetSetupPacket_NAND(page_addr As UInt32, page_offset As UInt16, transfer_size As UInt32, area As FlashArea) As Byte()
        ...
        setup_data(12) = CByte(NAND_DEV.PAGE_EXT And 255)
        setup_data(13) = CByte((NAND_DEV.PAGE_EXT >> 8) And 255)
        ...
        setup_data(18) = CByte(NAND_DEV.PAGE_COUNT And 255)
        setup_data(19) = CByte((NAND_DEV.PAGE_COUNT >> 8) And 255)
        setup_data(20) = TX_NAND_ADDRSIZE
        setup_data(21) = area 'Area (0=main,1=spare,2=all), note: all ignores layout settings
        Return setup_data
    End Function
    ...
    Public Function ReadBulk_NAND(page_addr As UInt32, page_offset As UInt16, count As UInt32, memory_area As FlashArea) As Byte()
        Try
            ...
            If MySettings.ECC_READ_ENABLED AndAlso (memory_area = FlashArea.Main) Then 'We need to auto-correct data uisng ECC
                ...
                If Not result Then Return Nothing
                Dim ecc_data() As Byte = NAND_ECC_ENG.GetEccFromSpare(oob_area_data, NAND_DEV.PAGE_SIZE, NAND_DEV.PAGE_EXT) 'This strips out the ecc data from the spare area
        ...
        Try
            ...
            ElseIf memory_area = FlashArea.Main Then
                If MySettings.ECC_WRITE_ENABLED Then
                    ...
                    NAND_ECC_ENG.SetEccToSpare(oob_data, ecc_data, NAND_DEV.PAGE_SIZE, NAND_DEV.PAGE_EXT)
                End If
            ...
            Do Until pages_to_write = 0
                Dim page_count_max As Integer = NAND_GetMaxPacketCount(NAND_DEV.PAGE_SIZE)
    ...
    Private Function NAND_GetMaxPacketCount(PageSize As UInt32) As Integer
    ...
    Public Sub PARALLEL_PORT_TEST()
        ...
        HardwareControl(FCUSB_HW_CTRL.VPP_DISABLE) 'Turns off adapter
        Utilities.Sleep(500)
